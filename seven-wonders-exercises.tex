...
%% Last-Updated: 2025-02-05T23:20:01+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anontrue
\newif\ifarxiv
...
Suppose that through a control surface, in a given crossing direction, there is a non-zero flux of momentum. Momentum is a quantity related to movement. Does this mean that the control surface must be in motion?
...
No, the control surface may also be completely still (in that coordinate system). For several reasons:
\begin{itemize}[nosep]
...
You have noticed that if you try to choose consistently a crossing direction on the whole band, you end up where you started but with an \emph{opposite} crossing direction. For this reason the Möbius band cannot be used as a control surface.

If we clip a part of it, that part can be used as a control surface. Or if we clip the band so that it gets an extra border, then it becomes the same as a twisted rectangle, and so it can be used as a control surface.
