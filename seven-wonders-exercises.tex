...
%% Last-Updated: 2025-02-26T22:22:12+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anontrue
\newif\ifarxiv
...
\item Now use a more precise constitutive relation between momentum and velocity. Keep the same parameters as in the previous question, but instead of Newton's $\yv= \yP/\ym$, implement the more precise formula
  \begin{equation*}
    \yv = \yP \frac{\yc}{\sqrt{\ym^{2} \yc^{2} + \abs{\yP}^{2}\,}}
  \end{equation*}
...
\item The improved constitutive relations is implemented by replacing the line\\
  ...
  \\ with\\
  \begin{tabular}{ll}
    \verb|v = c * P / sqrt(c^2 * m^2 + norm(P)^2);| & (Octave)
    \\
    \verb|v = c * P / sqrt(c**2 * m**2 + norm(P)**2)| & (Python)
  \end{tabular}
\\  and adding \\
  \begin{tabular}{ll}
  ...
  The vertical momentum $P_{z}(t)$ decreases \emph{linearly} with time, as in the previous case. This happens because at every time step the momentum is increased by the amount $\yG$, which is constant.

  The vertical position $z(t)$ at first has a curved graph, so it could be almost \emph{quadratic} there; but then it flattens out and becomes a line, so its dependence becomes \emph{linear}. This is unlike the previous case.
