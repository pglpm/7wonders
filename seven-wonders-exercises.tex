...
%% Last-Updated: 2026-02-18T01:23:54+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anonfalse
\newif\ifarxiv
...
\marginpar{\vspace{\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/graniteblocks3.jpg}%
\\\flushleftright\color{mpcolor}%
...
%
At a particular time $\ytf$ a car is moving uphill with an instantaneous velocity ${\yv(\ytf) = [\num{11.4}, \num{6.6}]\,\unit{m/s}}$ in the coordinate system $(x, z)$, as in the margin figure. Note that this is the car's velocity at that specific instant; we don't know the velocity at any earlier or later instant. The car (including driver) has mass \qty{1400}{kg}. Assume that air is negligible.
...
\\\flushleftright\color{mpcolor}%
The building is seen from the front}
%
A strong wind is hitting a building from one side, as illustrated in the margin figure; the wind is represented by the \textcolor{cyan}{light-blue dotted arrows}. As it hits the building's side, the wind's air is deflected upwards. We are interested in calculating the surface force that the wind exerts on the building's side.

At some distance from the building, the wind has a horizontal velocity ($x$ direction) of \qty{30}{m/s}. The deflected wind has a completely vertical velocity ($z$ direction).

There is a constitutive relation for the surface force $F$ in the case where matter is flowing through a static surface with perpendicular velocity $v$:
\begin{equation*}
...
\item[] $\mathcal{F}$, called the pressure vector or stress vector, is the force that there would be on the surface if matter weren't flowing through it.
\end{itemize}
In the present case this constitutive relation can be applied to the vertical surface represented by the vertical, \textcolor{yellow}{yellow dashed line} in the margin figure. The surface force there is called $\yF_{\textrm{left}}$. The pressure vector $\mathcal{F}$ is the horizontal force from atmospheric pressure:
\begin{equation*}
  \mathcal{F} = A\,p
  \qquad\text{with}\quad
p \approx \qty{1e5}{N/m^{2}} \ .
\end{equation*}

Proceed as follows in order to find the force that the wind exerts on the building's side:
\begin{enumerate}
\item Consider the imaginary control volume represented by the \textcolor{yellow}{yellow dashed rectangle} in the figure (it has a depth but it isn't visible since the figure shows it from the front).
\item Apply the balance of momentum to this control volume, focusing on the $x$-component.
\item Consider which surface forces must be added together.
\item Assume that the total momentum within the control volume is constant in time; that is, the wind is steady.
\item Assume that the air's mass is negligible.
\end{enumerate}
...
\item It looks like we have enough information if we focus on the \emph{right block} only, considered as one control volume. The physical principles we need are:
  ...
  If we denote with $\yP_{\textrm{R}}$ the momentum of the right block and with $\yF_{\textrm{R}}$ the \emph{total} surface force on it, we can write all the law and information above as follows:
  \begin{equation*}
    \begin{aligned}
...
\item Yes, now we do have enough information to calculate $\yF(\ytf)$. We are given the magnitude of the acceleration at time $\ytf$, and are told that it has the same direction of the velocity. We can find the acceleration components by multiplying the magnitude by a unit vector formed from the velocity:
  \begin{equation*}
    \begin{aligned}
      ...
      \\[1ex]
      &= [0.8223, 0.4761]\,\unit{m/s^{2}} \ .
    \end{aligned}
  \end{equation*}

  Since the car's mass is constant, we can then find the time derivative of the car's momentum by using Newton's relation between momentum and velocity:
\begin{equation*}
  \begin{aligned}
    \frac{\di\yP(\ytf)}{\dt} =
    ...
    \\
    &= \qty{1400}{kg} \cdot [0.8223, 0.4761]\,\unit{m/s^{2}}
    = [\num{1151}, \num{666.5}]\,\unit{N} \ .
  \end{aligned}
\end{equation*}
...
Denote with $\yP$ the total momentum contained in the imaginary control volume; with $\yF$ the total surface force exerted on its sides; and with $\yG$ the total volume force from gravity. The balance of momentum applies:
\begin{equation*}
\frac{\di\yP}{\dt} = \yF + \yG \ .
\end{equation*}

The text says to assume that the momentum is constant in time. This means
\begin{equation*}
  \frac{\di\yP}{\dt} = \zerov\,\unit{N}
\end{equation*}
and in particular, for the $x$-component,
\begin{equation*}
  \frac{\di P_{x}}{\dt} = \qty{0}{N} \ .
\end{equation*}

Moreover, the air's mass is negligible. This means that we can take the gravitational force ${\yG \approx \zerov\,\unit{N}}$ (note that there's only air within the control volume). In any case, the $x$-component of this force would be zero.

The total surface force $\yF$ is given by the sum of the forces on all six sides of the control volume (four visible in the figure as dashed lines, plus two on the front and back).
...
and adding
  ...
  \begin{itemize}[nosep]
  \item[] Octave \furl{https://pglpm.github.io/7wonders/scripts/tennisball_rP_relativity.m}{\texttt{tennisball\_rP\_relativity.m}},\item[] Python \furl{https://pglpm.github.io/7wonders/scripts/tennisball_rP_relativity.py}{\texttt{tennisball\_rP\_relativity.py}}.
  \end{itemize}
