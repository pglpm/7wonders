...
%% Last-Updated: 2026-06-17T08:30:58+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anonfalse
\newif\ifarxiv
...
\newcommand*{\updated}{\ifarxiv***\else\today%
%\enskip\DTMsetstyle{iso}\DTMcurrenttime
\fi}
\newcommand*{\propertitle}{The Seven Wonders of the World%\\[\jot]\Large Notes on 21st-century multiphysics
...
% \usepackage{pifont}
\iffalse\usepackage{exerciselabels}\fi
...
%
% \newcommand{\exercise}[2]{\section{#2}\label{#1}\addcontentsline{toc}{section}{#2}\marginpar{\vspace{-1.5\baselineskip}\hspace{-8em}\footnotesize Solution p.~\pageref{#1_sol}}}
\newcommand{\elabel}[1]{\label{#1}\marginpar{\vspace{-1.3\baselineskip}\hspace{-8em}\footnotesize Solution p.~\pageref{#1_sol}}}

\newcommand{\solution}[1]{\subsection*{\makebox[0pt][r]{\faIcon[regular]{lightbulb}\enskip}\ref{#1}}%
\addcontentsline{toc}{subsection}{\ref{#1}}%
\label{#1_sol}%
\marginpar{\vspace{-1\baselineskip}\hspace{-8em}\footnotesize Exercise p.~\pageref{#1}}
}% 
% \newcommand{\slabel}[1]{\ref{#1}%
% \addcontentsline{toc}{subsection}{\ref{#1}}%
% \label{#1_sol}%
% \marginpar{\vspace{-1\baselineskip}\hspace{-8em}\footnotesize Exercise p.~\pageref{#1}}%
% }% 
...
\newcommand*{\yns}{\ye_{\textrm{n}}} % unit normal vector
...
\newcommand*{\yFn}{\yF_{\textrm{n}}}
\newcommand*{\yFt}{\yF_{\textrm{t}}}
\newcommand*{\yFs}{\yF_{\textrm{s}}}
\newcommand*{\yFk}{\yF_{\textrm{k}}}
\newcommand*{\yFr}{\yF_{\textrm{other,t}}}
...
\newcommand*{\yGa}{\yG_{a}}
\newcommand*{\yGb}{\yG_{b}}
\newcommand*{\yEka}{\yE_{\textrm{k},a}}
\newcommand*{\yEkb}{\yE_{\textrm{k},b}}
\newcommand*{\yEpa}{\yE_{\textrm{p},a}}
\newcommand*{\yEpb}{\yE_{\textrm{p},b}}
...
\section{}\label{sec:chatgpt_laws}
...
Discuss the answer you get, based on what you have learned so far. (Note: if the output mentions a \enquote*{balance of boost momentum}, that's actually correct.)

Argue with the LLM and see where the discussion goes.


\section{}\elabel{sec:time_vel_primitive}
...
\section{}\elabel{sec:scalar_vect_quants}
...
\section{}\elabel{sec:correct_units}
...
\section{}\label{sec:explain_primitive}
...
\item If you have some large-language-model service, ask it to explain the difference between primitive and derived quantity, and between scalar and vector quantity. Find out weak or unsure points in its answer, given what you've learned so far.
\end{enumerate}

\section{}\elabel{sec:units_functions}
...
\solution{sec:time_vel_primitive}
...
\solution{sec:scalar_vect_quants}
...
\section{}\label{sec:veritasium_lightspeed}
...
\section{}\label{sec:personal_coords}
...
\section{}\elabel{sec:1D_motion_b}
...
\section{}\elabel{sec:3Dmotion_simple}
...
\begin{enumerate}[exerc]
\item Verify that the equation above is dimensionally consistent.
\item Calculate the expressions for velocity $\yv_{\text{S}}(t)$ and acceleration $\bm{a}_{\text{S}}(t)$.
\item Plot the three components of the velocity as functions of time for $t \in \clcl{-4, 4}\,\unit{s}$.
\end{enumerate}


\section{}\elabel{sec:1Dintegration}
...
\section{}\elabel{sec:extensive_vs_meas}
...
\section{}\elabel{sec:energy_meas_mass}
...
\section{}\label{sec:diff_mass_matter}
...
\section{}\label{sec:energy_vs_mass}
...
\solution{sec:extensive_vs_meas}
...
\section{}\elabel{sec:P_ball_light}
...
\section{}\elabel{sec:battery_wire}
...
\section{}\elabel{sec:fluxes_scalar}
...
\section{}\elabel{sec:surface_moves_if_flux}
...
\section{}\elabel{sec:fluxes_vector}
...
\section{}\elabel{sec:integrated_flux}
...
\solution{sec:surface_moves_if_flux}
...
\section{}\elabel{sec:calc_from_bal}
...
\section{}\label{sec:diff_bal_cons_llm}
\emph{With a colleague or a large language model:}
...
\section{}\label{sec:change_bal_or_const}
...
\item\label{item:team_A} One group of engineers show that if you keep the original constitutive relation~\eqref{eq:const_block}, but modify the conservation law for energy~\eqref{eq:energy_block} as follows:
  \begin{equation}
    \label{eq:energy_block_new}
     \frac{\di\ln\yE(t)}{\dt} = \frac{2}{b}\,\yH(t)
...
\section{}\elabel{sec:ideal_gas_const}
...
\section{}\elabel{sec:constants_previous}
...
\solution{sec:constants_previous}
...
\section{}\elabel{sec:calc_from_bal2-watertank}
...
\section{}\elabel{sec:calc_from_bal2-capacitors}
...
\section{}\elabel{sec:calc_from_bal2-capacitors-carbreak}
...
\section{}\elabel{sec:calc_from_bal2-capacitors-nuclearplant}
...
\section{}\label{sec:play_oscillating_reaction}
...
\section{}\elabel{sec:NO2_reaction_later}
...
\section{}\elabel{sec:calc_from_bal2-scripts}
...
\solution{sec:calc_from_bal2-watertank}
...
\solution{sec:calc_from_bal2-capacitors}
...
\solution{sec:calc_from_bal2-capacitors-carbreak}
...
\solution{sec:calc_from_bal2-capacitors-nuclearplant}
...
\solution{sec:NO2_reaction_later}

See \sects7.10--7.11 in the main text.



\solution{sec:calc_from_bal2-scripts}
...
\section{}\elabel{sec:kinds_matter_select}
...
\solution{sec:kinds_matter_select}
...
\section{}\elabel{sec:hexagon_fluxes}
...
\needspace{9\baselineskip}
\section{}\elabel{sec:tensile_spring}

\marginpar{\vspace{-1\baselineskip}\footnotesize\centering%
\includegraphics[width=0.75\linewidth]{images/material_piece_compr.jpg}%
}%

The margin figure shows a piece of material with momentum \textbf{ef}fluxes $\yFa$ and $\yFb$ across the surfaces $a$ and $b$. These momentum effluxes are depicted as \emph{compressive} surface forces exerted by the material.
\begin{enumerate}[exerc]
\item Draw an analogous figure depicting the \textbf{in}fluxes across the surfaces $a$ and $b$, that is, the forces experienced by the material.
\item Draw an analogous figure but in which $\yFa$ and $\yFb$ are depicted as \emph{tensile} surface forces instead.
\end{enumerate}



\needspace{15\baselineskip}
\section{}\elabel{sec:graniteblocks2}
...
\section{}\elabel{sec:graniteblocks3}
...
\needspace{7\baselineskip}
\section{}\elabel{sec:wind_building}
...
\section{}\elabel{sec:football_integral}
...
\section{}\elabel{sec:tennisballs_collision}

\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/tennisballs_collision.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
In a coordinate system $(t, x)$, at a particular time two tennis balls are at some distance from each other, and moving towards each other with opposite velocities of magnitude \qty{25}{m/s}. Each ball has mass \qty{0.060}{kg}. There is no gravity, no electromagnetic fields, no air.

\begin{enumerate}[exerc]
\item Consider a control volume consisting of both tennis balls. Which physical laws and physical principles do you need to use in order to calculate the total momentum content in this volume? Do not reason intuitively or heuristically: try to be methodical and exhaustive.
\end{enumerate}
...
\begin{enumerate}[exerc,resume*]
\item How much is the total momentum content in the control volume, after the collision? Explain which balance laws and other information you use to find the result.
\item Using only the balance of momentum, is it possible to find the magnitude of the velocity of each ball after the collision?using only
\end{enumerate}


\needspace{12\baselineskip}
\section{}\elabel{sec:bullet_block}
...
\section{}\elabel{sec:tennisball_fall}

At some time instant a tennis ball is at a height of \qty{10}{m} above ground, with a downward velocity of \qty{1.0}{m/s}. At a later instant of time the ball is at a height of \qty{8.0}{m} above ground. The tennis ball's mass is \qty{0.060}{kg}, its motion is purely vertical, there are no obstacles in its course, and air can be neglected.

How much is the velocity of the ball at the later instant of time, when it is \qty{8.0}{m} above ground?


\needspace{7\baselineskip}
\section{}\elabel{sec:sfriction_block}

\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/static_2people_noforces.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
Two people are handling a crate of \masse\ \qty{110}{kg}, as illustrated in the margin figure. The person on the left exerts a diagonal surface force $[350, 200]\,\unit{N}$; the one on the right, a horizontal surface force $[250, 0]\,\unit{N}$. The static-friction factor between the floor and the crate is $0.62$, whereas the kinetic-friction  factor is \num{0.45}. The gravitational acceleration is $g = \qty{9.8}{N/kg}$.
Assume that Amontons's laws of friction apply.

\begin{enumerate}[exerc]
\item If the crate is at rest, do the two people manage to set it into motion?
\item If the crate is moving, at some time $t$, with an instantaneous velocity of \qty{0.05}{m/s} in the positive-$x$ direction, how much is its horizontal acceleration?
\end{enumerate}


\needspace{10\baselineskip}
\section{}\elabel{sec:sfriction_gripper}

\marginpar{\vspace{-1\baselineskip}\centering\footnotesize\includegraphics[width=0.75\linewidth]{images/gripper_block.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
A robot-arm gripper must vertically lift a block of metal having \masse\ \qty{6.0}{kg}. The block must stay firmly between the gripper's fingers while they move vertically. The static-friction factor of the contact between block and fingers is \num{0.86}.
...
\begin{enumerate}[exerc]
\item The metal block is lifted with a constant upward velocity of \qty{0.10}{m/s}. What is the minimum magnitude of the normal, horizontal force that each gripper finger must exert in order to achieve this?

\item The metal block is then lifted with a constant upward acceleration of \qty{5.0}{m/s^2}. What is the minimum magnitude of the normal, horizontal force that each gripper finger must exert in order to achieve this?
\end{enumerate}


\section{}\elabel{sec:tennisball_RT}
...
\section{}\elabel{sec:tennisball_double}
...
\section{}\elabel{sec:tennisball_collision_theory}
...
\section{}\elabel{sec:tennisball_collision_script}
...
\section{}\label{sec:script_llm}

\emph{With a Large Language Model:}
...
\section{}\elabel{sec:spring_scripts}
...
\section{}\elabel{sec:state_scripts}
...
\section{}\elabel{sec:state_scripts_hooke}
...
\section{}\elabel{sec:gpt_inclinedplane}

\marginpar{\vspace{2\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/object_inclinedplane.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
The following prompt was given to a Large Language Model (ChatGPT~5.1):
\begin{framed}\footnotesize
    Use a coordinate system $(x, z)$ fixed with the ground, with $z$ upwards. The gravitational acceleration is \qty{9.8}{N/kg}. We have the following information below about a given time instant $t$:

    A small solid object of constant mass \qty{1}{kg} is in contact with an inclined plane at this time instant. The plane forms an angle of $\pu/4\,\unit{rad}$ with the ground, inclined from bottom-right to top-left. There cannot be friction between the object and the inclined plane.
...
\end{framed}
The LLM's output is displayed in table~\ref{tab:chatgpt_output_inclinedplane} on the next page.

\medskip

Determine whether the LLM's output and \enquote{analysis} are correct. Keep in mind, as always, that \emph{there isn't any logical reasoning underneath the output}: it's just built from word correlations computed from a huge corpus of text. Detect any sentences and conclusions that are false or meaningless for the present task.

\begin{table}[p]
  \caption{Large Language Model output}
  \label{tab:chatgpt_output_inclinedplane}
%  \vspace{-\baselineskip}
\begin{framed}\footnotesize
...
\noindent\textbf{1. Forces and constraint}

\begin{itemize}[nosep]
\item Mass: $\ym = \qty{1}{kg}$
\item Gravitational acceleration (downwards): $\yg = [0, -g]$ with $g = \qty{9.8}{m/s^2}$
\item Plane: inclined at angle $\theta = \pu/4$ above the horizontal, rising from right to left.
\item No friction: only gravity and a normal force $\yFn$ from the plane.
\end{itemize}
...
\noindent\textbf{2. Direction along the plane}

Let the plane's \textbf{up-slope} unit vector be
\begin{equation*}
  \bm{s}_{\text{up}}
...
The \textbf{down-slope} direction is the opposite:
\begin{equation*}
  \bm{s}_{\text{down}} 
  = -\bm{s}_{\text{up}}
  = [\cos\theta, -\sin\theta] \ .
...
\begin{equation*}
  \bm{a} = g\,\sin\theta\, \bm{s}_{\text{down}} \ ,
\end{equation*}
because $\ym = \qty{1}{kg}$ so $\bm{a} = \yF/\ym$ and the tangential component of weight is $\ym\,g\,\sin\theta$ along the plane.

\medskip

\noindent\textbf{3. Components in $(x,z)$ coordinates}

Using
\begin{equation*}
  \bm{s}_{\text{down}} = [\cos\theta, -\sin\theta]
\end{equation*}
we have
\begin{equation*}
\bm{a}
= g\,\sin\theta\, [\cos\theta, -\sin\theta]
= [g\,\sin\theta\,\cos\theta, -g\,(\sin\theta)^{2}] \ .
\end{equation*}
For $\theta = \pu/4$:
\begin{equation*}
\sin\theta = \cos\theta = \sqrt{2}/2 \ ,
\quad
...
a_{z} = -g/2 \ .
\end{equation*}
With $g = \qty{9.8}{m/s^2}$:
\begin{equation*}
a_x = +\qty{4.9}{m/s^2} \quad\text{(to the right)},
\qquad
a_z = -\qty{4.9}{m/s^2} \quad\text{(downwards)}.
\end{equation*}

So the acceleration vector at that instant is
\begin{equation*}
\bm{a} = [4.9, -4.9]\,\unit{m/s^2} \ .
\end{equation*}
\end{framed}
\end{table}
\FloatBlock
...
\needspace{9\baselineskip}
\solution{sec:tensile_spring}

\marginpar{\vspace{0\baselineskip}\footnotesize\centering%
\includegraphics[width=0.449\linewidth,align=c]{images/material_piece_compr_in.jpg}%
\hfill\includegraphics[width=0.541\linewidth,align=c]{images/material_piece_tens.jpg}%
\\\hspace{-2ex}\textbf{1.}\hspace{0.5\linewidth}\textbf{2.}%
}%
\begin{enumerate}[exerc]
\item See margin figure on the left. To depict the \textbf{in}fluxes we have changed the crossing directions represented by the \textcolor{red}{light-red wiggly arrows}. The influxes point in a direction opposite to the effluxes, by the principle of flux symmetry. Note that these are still \emph{compressive} forces.

\item See margin figure on the right. We are depicting \textbf{ef}fluxes, but now these are \emph{tensile}: they give to what's outside a momentum that points towards the surface.
\end{enumerate}


\needspace{9\baselineskip}
\solution{sec:graniteblocks2}
...
\item It looks like we have enough information if we focus on the \emph{right block} only, considered as one control volume. The physical principles we need are:
  ...
  \item The principle of \textbf{extensivity}; it says that the force occurring on the block's whole surface is the sum of the forces on individual parts of the surface. We need it because the balance of momentum requires the total force, but the problem is giving us separate forces.
...
\solution{sec:football_integral}
...
\solution{sec:tennisballs_collision}

\begin{enumerate}[exerc]
\item Given the information and the particular physical conditions, for instance the speeds involved are much lower than the speed of light, it seems that \textbf{Newton's constitutive relation for the momentum of matter}, \enquote*{\,${\yP = \ym\yv}$\,}, can be used here. But there is a hitch: this constitutive relation can be used only for a control volume within which matter has the same velocity $\yv$ everywhere. So we cannot apply the formula directly to our control volume, because some of the matter in it has a velocity, and some another. In fact, which velocity should we use in the formula,\enskip${v = +\qty{25}{m/s}}$\enskip or\enskip ${v = -\qty{25}{m/s}}$\,?

  In order to use Newton's relation for momentum we can use the \textbf{principle of extensivity}: the momentum in a control volume is equal to the sum of the momenta in any sub-volumes in which we decide to divide the original volume. In this case we can divide our control volume into two parts, in each of which the velocity of matter is uniform: the two sub-volumes consisting of each tennis ball separately. For each sub-volume we have the \masse\ content and the velocity.

  Let's denote the two sub-volumes with~L and~R. We apply Newton's relation to each, and then the principle of extensivity to find the total momentum $P$:
  \begin{equation*}
    \begin{aligned}
      &P_{\textrm{L}} = \ym_{\textrm{L}}\, v_{\textrm{L}} = \qty{1.5}{N\,s}
      &&\qquad\text{\footnotesize Newton's relation, sub-volume~L}
      \\
      &P_{\textrm{R}} = \ym_{\textrm{R}}\, v_{\textrm{R}} = \qty{1.5}{N\,s}
      &&\qquad\text{\footnotesize Newton's relation, sub-volume~R}
      \\
      &P = P_{\textrm{L}} + P_{\textrm{R}} = \qty{3.0}{N\,s}
      &&\qquad\text{\footnotesize extensivity principle}
    \end{aligned}
  \end{equation*}

\item In order to find the total momentum at a later time $\ytf$ we can apply the integral expression of the balance of momentum:
  \begin{equation*}
    P(\ytf) = P(\yti)
    + \int_{\yti}^{\ytf}\!F(t)\dt + \int_{\yti}^{\ytf}\!G(t) \dt \ ,
  \end{equation*}
  ...
  \begin{equation*}
    P(\ytf) = P(\yti)
    + \int_{\yti}^{\ytf}\!(\qty{0}{N})\dt + \int_{\yti}^{\ytf}\!(\qty{0}{N}) \dt
    = \qty{3.0}{N\,s} \ .
  \end{equation*}

\item No, using only the balance of momentum we cannot find the magnitude of each individual tennis ball. The balance only gives us the \emph{total} momentum.

  Newton's relation and the principle of extensivity are of no help either: applying them we know that
  \begin{equation*}
    \ym_{\textrm{L}}\, v_{\textrm{L}}(\ytf)
+ \ym_{\textrm{R}}\, v_{\textrm{R}}(\ytf) = P(\ytf) = \qty{3.0}{N\,s} \ ,
\end{equation*}
but this is one equation with two unknowns -- or more! In fact, the tennis balls could have exchanged some mass, so we could now even have different masses $\ym_{\textrm{L}}(\ytf)$ and $\ym_{\textrm{R}}(\ytf)$.

What about finding the individual momenta of the tennis balls, by considering them as two control volumes, applying the balance of momentum to each? It's a good thought; but in order to find the final momenta we'd need to know the surface force between the balls during the collision:
\begin{equation*}
  \begin{aligned}
    P_{\textrm{L}}(\ytf) &= P_{\textrm{L}}(\yti)
    + \int_{\yti}^{\ytf}\!F_{\textrm{R}\rightsquigarrow\textrm{L}}(t)\dt
    \\
    P_{\textrm{R}}(\ytf) &= P_{\textrm{R}}(\yti)
    + \int_{\yti}^{\ytf}\!F_{\textrm{L}\rightsquigarrow\textrm{R}}(t)\dt
    \\
    F_{\textrm{R}\rightsquigarrow\textrm{L}}(t) &=
    -F_{\textrm{L}\rightsquigarrow\textrm{R}}(t)
  \end{aligned}
\end{equation*}
...
We consider the bullet and the block together, as a single, deformable and possibly disconnected object; that's our control volume. Before time $\yti$ this control volume is made of two disconnected pieces; at time $\yti$ the two pieces get into contact with each other and start to merge; after time $\yti$ the control volume consists in one piece only.
...
\paragraph{Two control volumes} In this approach we consider the bullet as one control volume, and the block as another. Note that after time $\yti$ the control volume of the block has a hole, in which the control volume of the bullet resides.

The analysis is similar to the previous one, but since we have two control volumes we need to consider the balance of momentum for each one. Let's use the suffixes \enquote*{\,${}_{\textrm{bul}}$\,} and \enquote*{\,${}_{\textrm{blo}}$\,}:
\begin{equation*}
  \begin{aligned}
    &P_{\textrm{bul}}(\ytf) = P_{\textrm{bul}}(\yti)
    + \int_{\yti}^{\ytf}\!F_{\textrm{bul}}(t)\dt
    + \int_{\yti}^{\ytf}\!G_{\textrm{bul}}(t) \dt \ ,
    \\
    &P_{\textrm{blo}}(\ytf) = P_{\textrm{blo}}(\yti)
    + \int_{\yti}^{\ytf}\!F_{\textrm{blo}}(t)\dt
    + \int_{\yti}^{\ytf}\!G_{\textrm{blo}}(t) \dt \ .
\end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    ...
    = \qty{0}{N\,s} \ ,
    \\
    &P_{\textrm{bul}}(\ytf) = \ym_{\textrm{bul}}\, v_{\textrm{f}} \ ,
    \\
    &P_{\textrm{blo}}(\ytf) = \ym_{\textrm{blo}}\, v_{\textrm{f}} \ .
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    &G_{\textrm{bul}}(t) \equiv \qty{0}{N}\quad\text{between $\yti$ and $\ytf$}
    \qquad\implies\qquad
    \int_{\yti}^{\ytf}\!G_{\textrm{bul}}(t)\dt = \qty{0}{N\,s} \ ,
    \\
    &G_{\textrm{blo}}(t) \equiv \qty{0}{N}\quad\text{between $\yti$ and $\ytf$}
    \qquad\implies\qquad
    \int_{\yti}^{\ytf}\!G_{\textrm{blo}}(t)\dt = \qty{0}{N\,s} \ .
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  F_{\textrm{blo}}(t) = -F_{\textrm{bul}}(t)
  \quad\text{between $\yti$ and $\ytf$}
    \quad\implies\quad
    \int_{\yti}^{\ytf}\!F_{\textrm{blo}}(t)\dt
    =
    -\int_{\yti}^{\ytf}\!F_{\textrm{bul}}(t)\dt  \ .
...
\begin{equation*}
  \begin{aligned}
    ...
    + \int_{\yti}^{\ytf}\!F_{\textrm{bul}}(t)\dt
    + \int_{\yti}^{\ytf}\!G_{\textrm{bul}}(t) \dt
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}momentum balance bullet}}
    \\
    &P_{\textrm{blo}}(\ytf) = P_{\textrm{blo}}(\yti)
    + \int_{\yti}^{\ytf}\!F_{\textrm{blo}}(t)\dt
    + \int_{\yti}^{\ytf}\!G_{\textrm{blo}}(t) \dt
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}momentum balance block}}
    \\[1ex]
    &P_{\textrm{bul}}(\yti) = \qty{2.96}{N\,s}
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}Newton's constit. relation}}
    \\
    &P_{\textrm{blo}}(\yti) = \qty{0}{N\,s}
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}Newton's constit. relation}}
    \\
    &P_{\textrm{bul}}(\ytf) = \ym_{\textrm{bul}}\, v_{\textrm{f}}
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}Newton's constit. relation}}
    ...
    =
    -\int_{\yti}^{\ytf}\!F_{\textrm{bul}}(t)\dt
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}flux symmetry}}
    \\[1ex]
    &\int_{\yti}^{\ytf}\!G_{\textrm{bul}}(t)\dt = \qty{0}{N\,s}
    &\qquad&\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
  ...
  \end{aligned}
\end{equation*}
and the velocity $v_{\textrm{f}}$ can be found. For example, with several substitutions and rearrangements the first two equations become
\begin{equation*}
  \begin{aligned}
    \int_{\yti}^{\ytf}\!F_{\textrm{bul}}(t)\dt
    &=
    \ym_{\textrm{bul}}\, v_{\textrm{f}} - \qty{2.96}{N\,s}
    \\
    \ym_{\textrm{blo}}\, v_{\textrm{f}}
    &=
    - \int_{\yti}^{\ytf}\!F_{\textrm{bul}}(t)\dt
  \end{aligned}
\end{equation*}
and finally substituting the first in the second we arrive at $v_{\textrm{f}} \approx \qty{14}{m/s}$, as before.

\medskip

You notice that this approach was a little more roundabout than the previous one. We had the added difficulty of not knowing the forces $F_{\textrm{bul}}(t)$ and $F_{\textrm{blo}}(t)$, whose time dependence must be quite complicated. But the principle of symmetry of flux saved us, effectively eliminating the need to compute them explicitly. The previous approach bypassed this difficulty, because in that approach these were forces \emph{internal} to the control volume, and therefore they did not enter into its balance of momentum.



\solution{sec:tennisball_fall}

The solution of this problem turns out to require the solution of a system of \furl{https://mathworld.wolfram.com/First-OrderOrdinaryDifferentialEquation.html}{first-order ordinary differential equations}: it isn't enough to solve a system of linear or maybe quadratic equations. For this reason the problem could alternatively be approached with a simulation script.

First let's see why an ordinary system of equations isn't enough; then we discuss the proper solution.


\subsubsection{An inconclusive approach}
...
Our system of equations is therefore, with $g = \qty{9.8}{N/kg}$:
\begin{equation*}
  \begin{aligned}
    & P_{z}(\ytf) = P_{z}(\yti)
    + \int_{\yti}^{\ytf}\!\!F_{z}(t)\dt + \int_{\yti}^{\ytf} \!\!G_{z}(t)\dt
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}momentum balance}}
    \\
    & P_{z}(\ytf) = \ym\,v_{z}(\ytf)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}Newton's constit. relation for momentum}}
    \\
    & P_{z}(\yti) = \ym\,v_{z}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}Newton's constit. relation for momentum}}
    \\
    & G_{z}(t) \equiv -\ym\,g
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}constit. relation for gravitational force}}
    \\
    & v_{z}(\yti) = -\qty{1.0}{m/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
    & z(\yti) = \qty{10}{m}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
...
This is not quite a system of linear equations, because the quantity $G_{z}(t)$ is really a function of time, related by integration to the quantity $\int_{\yti}^{\ytf} \!\!G_{z}(t)\dt$. The same is true of $F_{z}(t)$. It is easy, however, to perform the integrations, because $G_{z}(t)$ is constant in time and $F_{z}(t)$ is constant and zero.

Doing the integrations and substituting we arrive at
\begin{equation*}
  \begin{aligned}
    & P_{z}(\ytf) = P_{z}(\yti) - \ym\,g\,(\ytf - \yti)
    \\
    & P_{z}(\ytf) = \ym\,v_{z}(\ytf)
    \\
    & P_{z}(\yti) = \ym\,v_{z}(\yti)
  ...
  \end{aligned}
\end{equation*}
where the unknown of interest to us is $v_{z}(\ytf)$, the ball's velocity at the later time instant.

Substituting into the first equation all remaining ones, and simplifying a common mass term $\ym$, we arrive at this equation:
\begin{equation*}
  v_{z}(\ytf) = v_{z}(\yti) - g\,(\ytf - \yti)
\end{equation*}
but we cannot really use it to find $v_{z}(\ytf)$, because the two time instants $\yti$ and $\ytf$, or at least their difference, are unknown.
...
\begin{equation*}
  z(\ytf) = z(\ytf) + \int_{\yti}^{\ytf} \!\!v_{z}(t)\dt
\end{equation*}
which relates the initial and final positions and times, and the velocity. But to use this equation \emph{we need to know the velocity $v_{z}(t)$ at all times between $\yti$ and $\ytf$}. This knowledge is required in order to perform the integration.

Let's therefore abandon this road and think more generally.


\subsubsection{Approach via differential equations}

The fact that we need to know the velocity at all times between the two instants means that we really need to time-integrate the system. That is, we must try to find the time dependence of $z(t)$, $v_{z}(t)$, $P_{z}(t)$, expressed by the system of differential equations
\begin{equation*}
  \begin{aligned}
    &\frac{\di P_{z}(t)}{\dt} = F_{z}(t) + G_{z}(t)
    \\
    &\frac{\di z(t)}{\dt} = v_{z}(t)
    \\
    &P_{z}(t) = \ym\,v_{z}(t)
    \\
    &F_{z}(t) = \qty{0}{N}
...
One way to proceed is by substituting Newton's relation into the first equation, and to take the time derivative of the second equation; we also substitute $F_{z}$ and $G_{z}$. We arrive at the two differential equations
\begin{equation*}
    \ym\frac{\di v_{z}(t)}{\dt} = -\ym\,g \ ,
    \qquad
 \frac{\di^{2} z(t)}{\dt^{2}} = \frac{\di v_{z}(t)}{\dt} \ .
\end{equation*}
The mass $\ym$ in the first disappears, and we can substitute the resulting expression for $\di v_{z}/\dt$ in the second:
\begin{equation*}
 \frac{\di^{2} z(t)}{\dt^{2}} = -g \ .
\end{equation*}
...
\begin{equation*}
  \frac{\di z(t)}{\dt} \equiv v_{z}(t) = B + 2C\,t \ ,
\end{equation*}
which is the velocity $v_{z}(t)$; and deriving once more we arrive at the constant $2C$, which must be equal to $-g$:
\begin{equation*}
  \frac{\di^{2} z(t)}{\dt^{2}} = 2C = -g
  \qquad\implies\qquad
  C = -\tfrac12 g \ .
\end{equation*}

We have thus solved the system of differential equations! In order to find as many of the constants $A$, $B$, $C$ as possible, as well as the final velocity $v_{z}(\ytf)$, we use the information given in the problem, substituting the relevant values of the time $t$:
\begin{equation*}
  \begin{aligned}
    z(\yti):&\quad & A + B\,\yti - \tfrac12 g\,{\yti}^{2} &= \qty{10}{m}
    \\
    z(\ytf):&\quad & A + B\,\ytf - \tfrac12 g\,{\ytf}^{2} &= \qty{8.0}{m}
    \\
    v_{z}(\yti):&\quad & B - g\,\yti &= -\qty{1.0}{m/s}
    \\
    &&  B - g\,\ytf &= v_{z}(\ytf)
  \end{aligned}
\end{equation*}

In order to find $v_{z}(\ytf)$ we need to solve the first three equations for $B$ and $\ytf$, then substitute in the last. The equation you obtain for $\ytf$ is quadratic, and you must choose the root for which $\ytf > \yti$. The final result is
\begin{equation*}
  v_{z}(\ytf) = -\sqrt{v_{z}(\yti)^{2} - 2\,g\,[z(\ytf) - z(\yti)]}
  \approx -\qty{6.3}{m/s} \ .
\end{equation*}

\medskip

In exercise~\ref{sec:tennisball_fall_energy} we'll see a way to solve this problem by means of the balance of energy: it will allow us to avoid facing differential equations.


\solution{sec:sfriction_block}

First let's find the unit normal to the contact surface between floor and crate, since it isn't given by the problem and it's necessary for defining what we mean by \enquote*{normal} and \enquote*{tangential} components. Given the geometry of the problem, the unit normal is
\begin{equation*}
  \yns = [0, 1] \ .
\end{equation*}
...
\paragraph{Step~1.}
We reckon all forces that appear in the problem; we assume air to be negligible as usual:
\begin{itemize}
\item The force from the person on the left; call it $\yFa = [350, 200]\,\unit{N}$.
\item The force from the person on the right; call it $\yFb = [250, 0]\,\unit{N}$.
\item The gravitational force, given by the usual constitutive relation:
  \begin{equation*}
    \yG = -\ym g [0, 1] = -[0, \num{1078}]\,\unit{N} \ .
  \end{equation*}
\item The contact force from floor to crate, which we write as the sum $\yFn + \yFs$ of a normal component and a tangential static-friction component.
\end{itemize}

\paragraph{Step~2.}
Amontons's law for static friction relates
\begin{enumerate*}[label=(\arabic*)]
\item the maximum magnitude that the tangential static-friction force $\yFs$ can have, and
\item the magnitude of the normal force $\yFn$ at the contact surface.
\end{enumerate*}
The next step in the problem is understanding which of these two forces we're trying to find, and which is given. In the present case it's the maximum tangential static friction $\yFs$ that we're trying to find. So we must find the normal force $\yFn$ in some other way.

The normal force can be found by observing that the crate does not move in the normal direction, which is the $z$-direction. So its $z$-momentum is constant in time: ${\di P_{z}/\dt = \qty{0}{N}}$. By the balance of $z$-momentum, the sum of the $z$-components of all surface and volume forces on the crate must therefore be zero:
\begin{equation*}
  F_{a,z} + F_{b,z} + G_{z} + F_{\textrm{n}} = \qty{0}{N}
  \quad\implies\quad
  F_{\textrm{n}} = -F_{a,z} - F_{b,z} - G_{z} = +\qty{878}{N} \ .
\end{equation*}

We did this calculation easily, noticing that the \enquote*{normal} direction is simply the $z$-direction. Let's do the same calculation in a more strict manner, which would work even if the normal direction were, say, diagonal to the coordinates. The reasoning is that the momentum in the direction of the unit normal does not change; therefore the normal component of the sum of all forces -- or equivalently the sum of the normal components of all forces -- must be zero. Remember that the normal component of any force $\yF$ is given by
\begin{equation*}
  (\yF \cdot \yns)\,\yns \ .
\end{equation*}
\needspace{5\baselineskip}
So in this case we want
\marginpar{\vspace{1\baselineskip}\centering\scriptsize%
\color{mpcolor}%
\begin{multline*}(\yFa\cdot\yns)\yns
={}\\ \bigl(
\begin{bsmallmatrix}350\\200\end{bsmallmatrix}
\cdot
\begin{bsmallmatrix}0\\1\end{bsmallmatrix}
\bigr)
\begin{bsmallmatrix}0\\1\end{bsmallmatrix}
\,\unit{N}
={}\\
...
\begin{equation*}
  \begin{gathered}
    \yF_{a,\textrm{n}} + \yF_{b,\textrm{n}} + \yG_{\textrm{n}} + \yFn = \zerov\,\unit{N}
    \\[1ex]
  \implies\quad
  \begin{aligned}[t]
    \yFn &= -\yF_{a,\textrm{n}} - \yF_{b,\textrm{n}} - \yG_{\textrm{n}}
    \\
    &= -(\yFa \cdot \yns)\,\yns
    -(\yFb \cdot \yns)\,\yns - (\yG \cdot \yns)\,\yns
    \\
    &= -[0, 200]\,\unit{N} - [0,0]\,\unit{N} - [0, -1078]\,\unit{N}
    \\
    &= [0, 878]\,\unit{N} \ .
  \end{aligned}
\end{gathered}
...
\paragraph{Step~3.}
Now that we have the normal component of the contact force from floor to crate, we can calculate the maximum value that static friction can have:
\begin{equation*}
  \abs{\yF_{\text{s,max}}} = \yfris \,\abs{\yFn}
  = 0.62 \cdot \qty{878}{N}
  = \qty{544}{N} \ .
\end{equation*}

\paragraph{Step~4.}
Finally let's find the tangential component of the sum of all other forces, and compare its magnitude with the maximum value above.

We already found the sum of all other forces:
\begin{equation*}
  \yF_{\text{other}} = \yFa + \yFb + \yG =
  [600, -878]\,\unit{N} \ .
\end{equation*}
The tangential component is therefore
\begin{equation*}
  \yFr = \yF_{\text{other}} - \yF_{\text{other,n}}
  = [600, -878]\,\unit{N} -  [0, -878]\,\unit{N}
  = [600, 0]\,\unit{N}
\end{equation*}
and its magnitude is \qty{600}{N}, which is larger than \qty{544}{N}:
\begin{equation*}
  \qty{600}{N} = \abs{\yFr} >
  \yfris\, \abs{\yFn} \approx \qty{544}{N} \ .
\end{equation*}
...
To determine the crate's horizontal acceleration $\di v_{x}/\dt$ we can use the balance of $x$-momentum, together with Newton's constitutive relation between momentum and velocity $P_{x} = \ym\,v_{x}$. Since the crate's mass is constant, we have
\begin{equation*}
  \frac{\di P_{x}}{\dt} = \frac{\di\ym\,v_{x}}{\dt}
  = \ym\frac{\di v_{x}}{\dt} \ .
\end{equation*}

The momentum fluxes and supply on the crate's control volume are $\yFa$, $\yFb$, $\yG$, together with the force at the contact surface with the floor, which we decompose into a normal component $\yFn$ and a tangential kinetic-friction component $\yFk$.

Amontons's law for kinetic friction relates $\yFk$ and $\yFn$. The normal component can be found, as in the previous part of this exercise, by consideration of the $z$-balance of momentum, which leads to
\begin{equation*}
  \yFn = [0, 878]\,\unit{N}
\end{equation*}
...
\begin{equation*}
  \begin{split}
    \yFk &= -\yfrik\,\frac{\yv}{\abs{\yv}}\,\abs{\yFn}
    \\[1ex]
    &= -0.45\cdot
    \frac{[0.05, 0]\,\unit{m/s}}{\qty{0.05}{m/s}}\cdot
    \qty{878}{N}
    \\
    & = -[395, 0]\,\unit{N} \ .
  \end{split}
...
Now we have all forces. Collect all their $x$-components in the balance of $x$-momentum; since $\yG$ and $\yFn$ are purely vertical, with zero $x$-component, let's directly omit them. We can then find the $x$-acceleration:
\begin{equation*}
  \begin{aligned}
    &\ym\frac{\di v_{x}}{\dt} =
    \frac{\di P_{x}}{\dt} =
    F_{a,x} + F_{b,x} + F_{\textrm{k},x}
    \\
    \implies\quad&
    \qty{110}{kg}\cdot \frac{\di v_{x}}{\dt} =
    \qty{350}{N} + \qty{250}{N} - \qty{395}{N}
    \\
    \implies\quad&
    \frac{\di v_{x}}{\dt} \approx \qty{1.9}{m/s^{2}} \ .
  \end{aligned}
\end{equation*}

Let's have a final overview of all physical laws and information we needed to solve this problem; note that even if the $x$-acceleration was found from the balance of $x$-momentum, we still needed the balance of $z$-momentum to find the normal force $\yFn$:
\begin{equation*}
  \begin{aligned}
    & \frac{\di P_{z}}{\dt} = F_{a,z} + F_{b,z} + F_{\textrm{k},x} + F_{\textrm{n},z} + G_{z}
    \\
    & \frac{\di P_{z}}{\dt} = \qty{0}{N} \ ,\quad v_{z} = \qty{0}{m/s}
    \\
    & \yG = -\ym\,g\,[0, 1]
    \\
    & \frac{\di P_{x}}{\dt} = F_{a,x} + F_{b,x} + F_{\textrm{k},x} + F_{\textrm{n},x} + G_{x}
    \\
    & P_{x} = \ym\,v_{x}
    \\
    & \yFk = -\yfrik\,\frac{\yv}{\abs{\yv}}\,\abs{\yFn}
...
The unit normal vectors of surfaces $a$ and $b$ are obviously
$[1, 0]$ and $[1, 0]$. Therefore \enquote*{normal component}, for both, means horizontal $x$-component, and \enquote*{tangential component} means vertical $z$-component.

\needspace{10\baselineskip}
\subsubsection{1.}

\marginpar{\vspace{-1\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/gripper_block_forces.jpg}%
\\\color{mpcolor}%
(Force $\yG$ not depicted)%
}
%
Let's first reckon the momentum fluxes and supply involved in the problem:
\begin{itemize}
\item Momentum supply from gravity to the block, obeying the usual constitutive relation:
  \begin{equation*}
    \yG = -\ym g [0,1] = -[0, 58.8]\,\unit{N} \ .
  \end{equation*}

\item Normal component of force exerted by the left finger; denote it $\yF_{a,\textrm{n}}$. It is purely horizontal.

\item Tangential, static-friction component of force exerted by the left finger; denote it $\yF_{a,\textrm{s}}$. It is purely vertical.

\item Normal component of force exerted by the right finger; denote it $\yF_{b,\textrm{n}}$. It is purely horizontal.

\item Tangential, static-friction component of force exerted by the right finger; denote it $\yF_{b,\textrm{s}}$. It is purely vertical.
\end{itemize}

Owing to of the symmetry of the problem we know that
\begin{equation*}
  \yF_{a,\textrm{n}} = -\yF_{b,\textrm{n}}
  \ ,\qquad
  \yF_{a,\textrm{s}} = \yF_{b,\textrm{s}} \ .
\end{equation*}

\medskip

Amontons's law let us find the maximal value of $\yF_{a,\textrm{s}}$ from $\yF_{a,\textrm{n}}$ or vice versa; and similarly for surface $b$. Which of these component do we need to find by other means? In this case the problem asks us to find the maximal magnitudes of $\yF_{a,\textrm{n}}$ and $\yF_{b,\textrm{n}}$, which we can find from Amontons's law provided that we first find $\yF_{a,\textrm{s}}$ and $\yF_{b,\textrm{s}}$ by other means.

In the present task we know that gripper and block are moving together with a \emph{constant} upward velocity ${v_{z} = \qty{0.10}{m/s}}$. By Newton's formula $P_{z} = \ym\,v_{z}$ this means that the \emph{rate of change} of $z$-momentum is zero: ${\di P_{z}/\dt = \qty{0}{N}}$. We can use this fact together with the balance of $z$-momentum and with the condition of symmetry about the contact forces:
\begin{equation*}
  \begin{gathered}
    \qty{0}{N} = \frac{\di P_{z}}{\dt} =
    F_{a,\textrm{s},z} + F_{b,\textrm{s},z} + G_{z}
    \ ,\qquad
    F_{a,\textrm{s},z} = F_{b,\textrm{s},z}
    \\[1ex]
    \implies\quad
    F_{a,\textrm{s},z} = F_{b,\textrm{s},z}
    = -G_{z} / 2 = +\qty{29.4}{N}
  \end{gathered}
\end{equation*}
In the balance of momentum we directly omitted the normal forces $\yF_{a,\textrm{n}}$, $\yF_{b,\textrm{n}}$ because their $z$-components are zero. So we have found that
\begin{equation*}
  \yF_{a,\textrm{s}} = \yF_{b,\textrm{s}} = [0, 29.4]\,\unit{N}
\end{equation*}
they're directed upwards as expected.

\medskip

Now that we have $\yF_{a,\textrm{s}}$  we can find the maximal values of $\yF_{a,\textrm{n}}$ by means of Amontons's law:
\begin{equation*}
  \abs{\yF_{a,\textrm{s}}} < \yfris\,\abs{\yF_{a,\textrm{n}}}
  \quad\implies\quad
  \abs{\yF_{a,\textrm{n}}} > \frac{\abs{\yF_{a,\textrm{s}}}}{\yfris}
  =  \frac{\qty{29.4}{N}}{0.86} \approx
  \qty{34}{N} \ .
\end{equation*}
Obviously the same is true for $\yF_{b,\textrm{n}}$ since it has the same magnitude:
\begin{equation*}
  \abs{\yF_{a,\textrm{n}}} > \qty{34}{N} \ ,
  \qquad
  \abs{\yF_{b,\textrm{n}}} > \qty{34}{N} \ .
\end{equation*}


\subsubsection{2.}

How is this case different from the previous one? The gripper and block are \emph{accelerating} upwards, and this means that the \emph{rate of change} of $z$-momentum is \emph{non-zero}. We have instead
\begin{equation*}
  \frac{\di P_{z}}{\dt} = \ym\frac{\di v_{z}}{\dt}
  = \qty{0.6}{kg} \cdot \qty{5.0}{m/s^{2}} = \qty{30}{N} \ ,
\end{equation*}
constant in time.

The reasoning and physical laws to find the gripper's normal forces $\yF_{a,\textrm{n}}$ and $\yF_{b,\textrm{n}}$ is exactly the same as before: first find the tangential, static-friction components from the balance of $z$-momentum, then find the minimum magnitudes of the normal components from Amontons's law. But in this case, when we use the balance of $z$-momentum, we must keep account for the non-zero $\di P_{z}/\dt$.
...
\begin{equation*}
  \begin{gathered}
    ...
    \implies\quad
    F_{a,\textrm{s},z} = F_{b,\textrm{s},z}
    = \frac12 \biggl(\frac{\di P_{z}}{\dt} - G_{z}\biggr)
    = \frac12 (\num{30} + \num{58.8})\,\unit{N}
    = \qty{44.4}{N} \ .
  \end{gathered}
\end{equation*}
...
\begin{equation*}
  \abs{\yF_{a,\textrm{n}}} > \frac{\abs{\yF_{a,\textrm{s}}}}{\yfris}
  =  \frac{\qty{44.4}{N}}{0.86} \approx
  \qty{52}{N} \ .
\end{equation*}
And analogously for side $b$.


\solution{sec:tennisball_RT}
...
\solution{sec:tennisball_double}
...
\solution{sec:tennisball_collision_theory}
...
\solution{sec:tennisball_collision_script}
...
\solution{sec:spring_scripts}
\mynotew{Coming soon}

\solution{sec:state_scripts}
\mynotew{Coming soon}

\solution{sec:state_scripts_hooke}
\mynotew{Coming soon}


\solution{sec:gpt_inclinedplane}
...
\begin{equation*}
    \frac{\di P_{x}}{\dt} = F_{x} + G_{x} \ ,
    \qquad
    \frac{\di P_{z}}{\dt} = F_{z} + G_{z} \ .
\end{equation*}

We need to connect momentum and its rate of change to velocity and acceleration. Newton's relation momentum and velocity can be used for this, since we have a solid object made of matter. We can also take the derivative of this relation, to obtain an acceleration:
\begin{equation*}
    P_{x} = \ym\,v_{x}
    \enskip\implies\enskip
    \frac{\di P_{x}}{\dt} = \ym \frac{\di v_{x}}{\dt} \ ,
    \qquad
    P_{z} = \ym\,v_{z}
    \enskip\implies\enskip
    \frac{\di P_{z}}{\dt} = \ym \frac{\di v_{z}}{\dt} \ .
...
The only additional information given in the question is that there is no friction between plane and object. Friction is the tangential component $\yFt$ of the surface force (momentum flux) ${\yF = [F_{x}, F_{z}]}$ from the plane to the object. The question therefore says that this tangential component must be zero: ${\yFt = \zerov\,\unit{N}}$. Let's write this condition in terms of the $x$- and $z$-components of $\yF$. To do so we need the unit normal vector $\yns$ to the plane, directed towards the solid object. We can find vector considering that its components must be positive, because it points to the positive-$x$ and -$z$ directions, they must be equal since it's inclined by $\pu/4\,\unit{rad}$, and their squares must sum up to 1:
\begin{equation*}
  \yns = \frac{1}{\sqrt{2}}\,\begin{bmatrix}1 \\ 1 \end{bmatrix} \ .
\end{equation*}

The normal component of the force from the plane $\yF$ is therefore
\begin{equation*}
  \begin{aligned}
    \yFn &= (\yFn \cdot \yns)\, \yns
    \\ &= \frac{1}{\sqrt{2}}\,(F_{x}\cdot 1 + F_{z} \cdot 1)\,
    \frac{1}{\sqrt{2}}\,\begin{bmatrix}1 \\ 1 \end{bmatrix}
    \\ &= \frac12\,(F_{x} + F_{z})\,\begin{bmatrix}1 \\ 1 \end{bmatrix}
    \ .
  \end{aligned}
\end{equation*}
This results says that the $x$- and $z$-components \emph{of the normal component $\yFn$} are equal. This make sense because $\yFn$ must be inclined by $\pu/4\,\unit{rad}$. Note that we haven't said anything about the full force $\yF$ yet.

The tangential component of $\yF$ is
\begin{equation*}
  \begin{aligned}
    \yFt &= \yF - \yFn
    \\ &=
...
\begin{equation*}
  ...
  F_{x} = F_{z} \ .
\end{equation*}
This result also makes sense: it simply says that the components of the force $\yF$ from the plane to the object must be equal; and this is indeed the condition that $\yF$ be normal to the plane, without any tangential component.

Summarizing all physical information we have about the time instant $t$:
\begin{equation*}
  \begin{aligned}
    &\frac{\di P_{x}}{\dt} = F_{x} + G_{x}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}balance $x$-momentum}}
    \\
    &\frac{\di P_{z}}{\dt} = F_{z} + G_{z}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}balance $z$-momentum}}
    \\
    &\frac{\di P_{x}}{\dt} = \ym \frac{\di v_{x}}{\dt}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}from Newton's relation}}
    \\
    &\frac{\di P_{z}}{\dt} = \ym \frac{\di v_{z}}{\dt}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}from Newton's relation}}
    \\
  &G_{x} = \qty{0}{N}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}gravit. force}}
  \\
  &G_{z} = -\qty{9.8}{N}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}gravit. force}}
    \\
    &F_{x} = F_{z}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}no friction}}
  \end{aligned}
\end{equation*}
The equations above can be combined into these final three, writing explicitly the object's mass $\ym = \qty{1}{kg}$:
\begin{equation*}
  \begin{aligned}
    &\qty{1}{kg} \cdot \frac{\di v_{x}}{\dt} = F_{x}
    \\
    &\qty{1}{kg} \cdot \frac{\di v_{z}}{\dt} = F_{z} - \qty{9.8}{N}
    \\
    &F_{x} = F_{z}
  \end{aligned}
\end{equation*}
We have \emph{three} equations in \emph{four} unknowns: the two components of the acceleration\enskip$\di v_{x}/\dt, \di v_{z}/\dt$\,\enskip and the two components of the force from the plane\enskip$F_{x}, F_{z}$\,.\enskip The problem is indeterminate.

Let's go back to the question in the prompt: \enquote*{is it possible to calculate the acceleration of the object at this time instant?} The answer is apparently \emph{No}. The information given in the problem is not enough. To find numerical values we'd need to be given one component of the acceleration, or one component of the force.

For example it could be that at this time $t$ the $z$-component of the force is zero. From the equations above this would mean that the $z$-component is also zero, the $x$-acceleration is zero, and the $z$-acceleration is \qty{-9.8}{m/s^2}. This could indeed be the case if the object was falling onto the plane, and $t$ is the instant where the object has just begun to touch the surface, so that the force from the surface is still zero or extremely small. Or it could be the case if the object is bouncing off the plane, and $t$ is the last instant of contact, when the force from the plane is going back to zero.

\needspace{7\baselineskip}
\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=0.75\linewidth]{images/tennisball_bounce45.jpg}%
\\\color{mpcolor}\flushleftright%
Snapshot of a tennis ball bouncing on a surface%
}
%
It could also be that the $x$- and $z$-components of the force are extremely large, say \qty{100}{N}. In this case the $x$- and $z$-accelerations would also be positive and large, approximately \qty{+100}{m/s^2} and \qty{+90}{m/s^2}. This could be the case if the object is in the middle of bouncing on the surface, when the surface exerts extremely high reaction forces (indeed objects compress to a small or high degree when bouncing).

\bigskip

Let's now analyse some statements in the LLM's output. Again, we must not think of these statements as coming from some kind of reasoning: \emph{they don't}. They're only produced by long-range correlations among words matched from some corpus of text.

One statement in particular is \emph{false} in the present context:
\begin{quote}\footnotesize
  Because the object is constrained to remain in contact with the plane\textellipsis
\end{quote}
nowhere in the prompt it was stated that the object is constrained to remain in contact with the plane. The prompt only stated that the object \enquote{is in contact with an inclined plane at this time instant}. So the \enquote{because\textellipsis} in the LLM's output sounds even funny. Subsequent statements in the output are likewise false, for instance \enquote{The normal component of the acceleration is zero}.

The rest of the output is more or less self-consistent and consistent with the \emph{false} statement above; it is therefore all false. The large language model probably extracted highly correlated text from a corpus of physics exercises similar to the present one. Many of those texts likely made the additional assumption of \enquote{constrained to remain in contact}, and therefore this also entered the LLM's output by correlation.
...
\needspace{5\baselineskip}
\section{}\elabel{sec:skydiver}

\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/skydive2.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
A skydiver of mass \qty{70}{kg} jumps with zero initial velocity at an altitude of \qty{3000}{m}. At \qty{1000}{m} the skydiver has a fully vertical velocity of \qty{55}{m/s} (around \qty{200}{km/h}), ready to deploy the parachute. Assume that the internal energy of the skydiver is the same at \qty{1000}{m} as at \qty{3000}{m}.
\begin{enumerate}[exerc]
\item How much is the time-integrated energy influx to the skydiver from the surrounding air?
\item Can you tell how much are the time-integrated heat flux and the mechanical work, separately, given the information in this problem?
\end{enumerate}


\needspace{12\baselineskip}
\section{}\elabel{sec:bullet_block_energy}

\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=\linewidth]{images/bullet_block.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
Consider again exercise~\ref{sec:bullet_block}:
...
\begin{enumerate}[exerc]
\item Do you have enough information for finding the common final velocity $v_{\textrm{f}}$ of bullet \amp\ block by means of the balance of energy? If so, find it.

\item If you don't have enough information for using the balance of energy, then try to use the result ${v_{\textrm{f}} = \pm \qty{14}{m/s}}$ from solution~\ref{sec:bullet_block_sol} to make hypotheses about the missing information about energy.
\end{enumerate}

\section{}\elabel{sec:tennisball_fall_energy}

Consider again exercise~\ref{sec:tennisball_fall}:

At some time instant a tennis ball is at a height of \qty{10}{m} above ground, with a downward velocity of \qty{1.0}{m/s}. At a later instant of time the ball is at a height of \qty{8.0}{m} above ground. The tennis ball's mass is \qty{0.060}{kg}, its motion is purely vertical, there are no obstacles in its course, and air can be neglected.

The original exercise asks how much is the velocity of the ball at the later instant of time, when it is \qty{8.0}{m} above ground. Can you find this velocity using only the balance of energy? if so, find it.



\needspace{9\baselineskip}
\section{}\elabel{sec:skaters_away}

\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=0.75\linewidth]{images/skaters_away.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
Two ice skaters are holding each other, completely still, at some time $\yti$. Then they push each other and at a later time $\ytf$ they're moving in opposite directions with purely horizontal velocities. Together they have burned \qty{76}{J} of internal energy in total.

Assume there's no friction or heat flux between ice and the skaters, air is negligible, and the gravitational potential energy of each skater is constant. The skater on the left has \masse\ \qty{70}{kg}, the one on the right \qty{60}{kg}.

What are the velocities of the skaters at time $\ytf$?


\section{}\elabel{sec:imagine_momentum_energy}
...
\item At an initial time $\yti$ and at a final time $\ytf$, the momentum is zero.
\item Gravity and other momentum supplies are absent or negligible.
\item No external fluxes of momentum or of energy occur between times $\yti$ and $\ytf$.
\item The internal energy at final time $\ytf$ is lower than at the initial time $\yti$.
\item The kinetic energy is zero at initial time $\yti$ is zero, and it is non-zero (and obviously positive) at final time $\ytf$.
\end{enumerate}


\section{}\elabel{sec:idealgas_kineticenergy}
...
\needspace{6\baselineskip}
\section{}\elabel{sec:ramp_cart}

\marginpar{\vspace{0\baselineskip}\centering\footnotesize\includegraphics[width=1\linewidth]{images/ramp_cart_ice.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
A rigid ramp of mass \qty{20}{kg} and height \qty{1}{m} is at rest on a static layer of ice. At the top of the ramp there's a small cart of mass \qty{1}{kg}, equipped with small wheels and also at rest. The cart is released, and at a later time $\ytf$ it is at the bottom of the ramp, moving in a horizontal direction.

There is no friction between the ramp and the layer of ice. Assume that there are no heat fluxes, air is negligible, and the internal energies of ramp and cart are constant. Consider the cart as a small object identified simply by its position $\yr$.

What are the horizontal velocities (complete with correct sign) of cart and of ramp at time $\ytf$?


\clearpage
\addsec{Example solutions}

\solution{sec:skydiver}

\begin{enumerate}[exerc]
\item We can consider the skydiver as a control volume, and use the integral expression of the balance of energy: the integral expression seems more convenient because the problem involves two different times. Call $\yti$ when the skydiver jumps, and $\ytf$ when the skydiver reaches \qty{1000}{m} altitude:
  \begin{equation*}
    \yE(\ytf) = \yE(\yti)
    + \int_{\yti}^{\ytf}\!\!\yH(t)\dt + \int_{\yti}^{\ytf} \!\!\yA(t)\dt \ ,
  \end{equation*}
...
Do we need the constitutive relation for the energy flux in matter (that is $\yH = \yQ + \yF\cdot \yv$)? The problem asks for the \emph{energy influx}, and not for the heat influx or the mechanical power separately; so maybe we manage to do without this constitutive relation. Let's see.

Collecting the equations above and the information given in the problem, we obtain this system of equations, where $g = \qty{9.8}{J/(kg\,m)}$:
\begin{equation*}
  \begin{aligned}
    & \yE(\ytf) = \yE(\yti)
    + \int_{\yti}^{\ytf}\!\!\yH(t)\dt + \int_{\yti}^{\ytf} \!\!\yA(t)\dt
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy balance}}
    \\[1ex]
    & \yE(\yti) =
    \yU(\yti) + \frac{1}{2}\,\ym\, v_{z}(\yti)^{2} + \ym\, g\, z(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for matter, initial time}}
    \\
    & \yE(\ytf) =
    \yU(\ytf) + \frac{1}{2}\,\ym\, v_{z}(\ytf)^{2} + \ym\, g\, z(\ytf)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for matter, final time}}
    % \\[1ex]
    %  & \yH(\yti) = \yQ(\yti) + \yF(\yti)\cdot v_{z}(\yti)
    % &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy flux for matter, initial time}}
    % \\
    %  & \yH(\ytf) = \yQ(\ytf) + \yF(\ytf)\cdot v_{z}(\ytf)
    % &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy flux for matter, final time}}
    \\[1ex]
     & \yA = \qty{0}{J/s}
     &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}no energy supply in inertial system}}
     \\[1ex]
     & \ym = \qty{70}{kg}
     &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
     \\
...
\begin{equation*}\textstyle
  ...
  \ym
\end{equation*}
but maybe we still manage to find the unknown of interest, which is the time-integrated influx of energy:\enspace$\int_{\yti}^{\ytf}\!\!\yH(t)\dt$\,.

Indeed replacing into the first equation all the remaining ones, and then rearranging and simplifying, we find
\begin{equation*}
  \begin{aligned}
    \int_{\yti}^{\ytf}\!\!\yH(t)\dt &=
    \frac{1}{2}\,\ym\, v_{z}(\ytf)^{2} + \ym\, g\, z(\ytf)
    - \frac{1}{2}\,\ym\, v_{z}(\yti)^{2} - \ym\, g\, z(\yti)
    \\
    &= \qty{105875}{J} + \qty{686000}{J}
    - \qty{0}{J} - \qty{2058000}{J}
    \approx -\qty{1300000}{J} \ .
  \end{aligned}
\end{equation*}
This is a negative influx, which means that a net amount of positive energy flows from the skydiver to the surrounding air. This is the effect of friction, which does \emph{negative} work on the skydiver, and heats both skydiver and air.

In the system of equations above, ten equations in eleven unknowns, there must be some quantities that depend on a free variable. Which ones are they? They are $\yE(\ytf)$, $\yE(\yti)$, $\yU(\yti)$, $\yU(\ytf)$. We don't know their values; but once one of them is specified, we can find the remaining three.

\item No, we cannot find the time-integrated heat flux and the mechanical work separately. In order to find them we would need to know either the constitutive relation for the surface force (momentum flux) between the skydiver and the air, or the heat flux.
\end{enumerate}


\solution{sec:bullet_block_energy}

\needspace{2\baselineskip}
\begin{enumerate}[exerc]
\item 
\end{enumerate}
Let's see if this problem, previously solved in~\ref{sec:tennisball_fall_sol} using the balance of momentum, can be solved using the balance of energy. In the previous solution we explored two approaches: first considering bullet \amp\ block as one control volume (a single object), and then as two control volumes. Let's explore only the first here.
...
Regarding constitutive relations, we can use those for the content and flux of energy of matter because bullet \amp\ block are a bunch of matter. These constitutive relations must be used at the initial and at the final times. Also, since the bullet and block have initially different velocity and may have initially different internal energies, we need to use the principle of extensivity to find the initial total energy.

The text says that gravity can be neglected, so the gravitational potential energy is always zero. Note that if the problem had been two-dimensional and with gravity, then we could \emph{not} have used the formula $\ym g z$ for the potential energy of the block and for that of block and bullet together, because these are vertically extended objects without a single $z$ coordinate.

The text gives information about velocities and masses, which can be useful for the kinetic energy. The text also says that there are no surface forces, besides those between bullet and block. This means that there is no flux of mechanical power to the bullet \amp\ block as a whole. Note that if there had been external surface forces, we could \emph{not} have used the formula $\yF\cdot\yv$ for mechanical power directly, because bullet and block have different velocities, and the material within the block might have a yet different velocity during the perforation by the bullet.

The text does not give information about heat fluxes and internal energies.

Putting all physical laws and text information together we arrive at this system of equations:
Together with the information given in the text, we then have the following set of equations, where $g = \qty{9.8}{J/(kg\,m)}$:
\begin{equation*}
  \begin{aligned}
    & \yE(\ytf) = \yE(\yti)
    + \int_{\yti}^{\ytf}\!\!\yH(t)\dt
    ...
    & \yE_{\textrm{bul}}(\yti) =
    \yU_{\textrm{bul}}(\yti)
    + \tfrac{1}{2}\,\ym_{\textrm{bul}}\, v_{\textrm{bul}}^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for bullet, initial time}}
    \\[1ex]
    & \yE_{\textrm{blo}}(\yti) =
    \yU_{\textrm{blo}}(\yti)
    + \tfrac{1}{2}\,\ym_{\textrm{blo}}\, v_{\textrm{blo}}^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for block, initial time}}
    \\
    &E(\yti) = E_{\textrm{bul}}(\yti) + E_{\textrm{blo}}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}extensivity, initial energy}}
    \\
    & \yE(\ytf) =
    \yU(\ytf)
    + \tfrac{1}{2}\,(\ym_{\textrm{bul}} + \ym_{\textrm{blo}})\, v_{\textrm{f}}^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for matter, final time}}
    \\[1ex]
     & \yH(t) = \yQ(t) + \underbracket[0.5pt]{\text{\footnotesize mechanical power}}_{\qty{0}{J/s}}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy flux for matter}}
     \\[1ex]
    & v_{\textrm{bul}} = \qty{370}{m/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
    & v_{\textrm{blo}} = \qty{0}{m/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
    & \ym_{\textrm{bul}} = \qty{0.0080}{kg}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
    & \ym_{\textrm{blo}} = \qty{0.2}{kg}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
  \end{aligned}
\end{equation*}

Substituting all equations into the balance of energy and rearranging, we arrive at the following expression for the final velocity of bullet \amp\ block:
\begin{equation*}
  \begin{aligned}
    \tfrac12\,(\ym_{\textrm{bul}} + \ym_{\textrm{blo}})\, v_{\textrm{f}}^{2}
    &= \yU_{\textrm{blo}}(\yti) + \yU_{\textrm{bul}}(\yti) - \yU(\ytf)
    + \tfrac{1}{2}\,\ym_{\textrm{bul}}\, v_{\textrm{bul}}^{2}
    + \tfrac{1}{2}\,\ym_{\textrm{blo}}\, v_{\textrm{blo}}^{2}
    + \yhe
    \\[1ex]
    \implies\quad
    (\qty{0.104}{kg})\cdot v_{\textrm{f}}^{2}
    &= \yU_{\textrm{blo}}(\yti) + \yU_{\textrm{bul}}(\yti) - \yU(\ytf)
    +\yhe
    + \qty{547.6}{J}
  \end{aligned}
\end{equation*}
where the time-integrated heat flux is denoted by $\yhe$.
...
Now suppose that we had assumed, maybe from a wrong intuition, that the total internal energy does not change, so that $\yU_{\textrm{blo}}(\yti) + \yU_{\textrm{bul}}(\yti) - \yU(\ytf) = \qty{0}{J}$, and also that the heat flux is zero: $\yhe = \qty{0}{J}$. Then  we would have arrived at the result ${v_{\textrm{f}} = \pm \qty{73}{m/s}}$. But the result obtained from the balance of momentum in the solution~\ref{sec:bullet_block_sol} was ${v_{\textrm{f}} = \pm \qty{14}{m/s}}$, and it was obtained without making any intuitive assumptions, so it's the correct result. This means that there actually is an increase in internal energy, or that some heat has been released.

\needspace{2\baselineskip}
\begin{enumerate}[exerc,resume]
\item 
\end{enumerate}
According to the solution obtained with the balance of momentum, the final kinetic energy of bullet \amp\ block is
\begin{equation*}
  \tfrac12\,(\ym_{\textrm{bul}} + \ym_{\textrm{blo}})\, v_{\textrm{f}}^{2}
  \approx \qty{20.384}{J} \ .
\end{equation*}
...
\begin{equation*}
  \yU_{\textrm{blo}}(\yti) + \yU_{\textrm{bul}}(\yti) - \yU(\ytf)
  + \yhe
  = \qty{20.384}{J} - \qty{547.6}{J}
  \approx -\qty{530}{J} \ .
\end{equation*}

So possible hypotheses are:
\begin{itemize}
\item There was no heat flux, and the total internal energy increased by \qty{530}{J}:
  \begin{equation*}
    \yU(\ytf) = [\yU_{\textrm{blo}}(\yti) + \yU_{\textrm{bul}}(\yti)]
    +\qty{530}{J} \ .
  \end{equation*}
  This scenario seems plausible; we expect the temperatures of both bullet and block to increased during the perforation.
\item There was no change in internal energy, and an amount \qty{530}{J} of heat was \emph{released} (negative heat influx because of the minus sign):
  \begin{equation*}
    \yhe = -\qty{530}{J} \ .
  \end{equation*}
This scenario also has some plausibility. If there is air, heat could have been released to it.
\item There could also have been a partial increase in internal energy, and partial release of heat, for a total of \qty{530}{J}.
\end{itemize}


\solution{sec:tennisball_fall_energy}

In the previous solution~\ref{sec:tennisball_fall_sol} of this exercise we used the balance of momentum together with the kinetic relationship between position and velocity, effectively solving a differential equation. Let's see if the balance of energy can lead us to the solution, and if it can do so more easily.

We consider the tennis ball as our control volume. It makes sense to use the integral expression of the balance of energy, because we have two distinct times. In the balance let's directly omit the energy supply $\yA$, which is zero in an inertial coordinate system. A tennisball is a bunch of matter, so we can use the constitutive relations for the content and energy of matter; we need to use them at time $\yti$ as well as at time $\ytf$.

What kind of information about energy does the text give us? Besides velocities and heights, which can be used for kinetic energy and gravitational potential energy, the text says that there are no obstacles and air can be neglected. Therefore the total surface force $\yF$ is zero, and also the mechanical power $\yF\cdot\yv$ is zero. Since the tennis ball is falling in a vacuum, let's assume that there's no heat flux $\yQ$ to or from the ball either; this assumption is not completely guaranteed, because the ball could emit electromagnetic radiation into a vacuum; but it seems reasonable.
...
\begin{equation*}
  \begin{aligned}
    ...
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for matter, final time}}
    \\
     & \yH(t) = \yQ(t) + F_{z}(t)\cdot v_{z}(t)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy flux for matter}}
     \\[1ex]
    & F_{z}(t) = \qty{0}{N}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on momentum flux}}
    \\
    & \yQ(t) = \qty{0}{J/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on heat flux}}
    \\
    & v_{z}(\yti) = -\qty{1.0}{m/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
...
\begin{equation*}
  \begin{aligned}
    \frac{1}{2}\,\ym\, v_{z}(\ytf)^{2}
    &= \yU(\yti) -\yU(\ytf)
    + \ym\, g\, z(\yti) - \ym\, g\, z(\ytf)
    + \frac{1}{2}\,\ym\, v_{z}(\yti)^{2}
    \\
    \implies\quad
    (\qty{0.030}{kg})\cdot v_{z}(\ytf)^{2}
    &= \yU(\yti) -\yU(\ytf)
    + \qty{1.176}{J}
    + \qty{0.03}{J}
  \end{aligned}
...
This could be solved for the final velocity $v_{z}(\ytf)$ if we knew the difference between the initial and final internal energies of the tennis ball. \emph{Intuitively} we expect this difference to be zero, so that
\begin{equation*}
  v_{z}(\ytf) \approx \pm\qty{6.3}{m/s}
  \quad\text{(if the internal energy is the same at the two times).}
\end{equation*}

We see that the balance of energy led us to \emph{part} of the solution without the need to solve differential equations. But pay attention to these uncertain elements of this solution:
\begin{itemize}[nosep]
\item The balance of energy does not tell us the \emph{sign} of the final velocity: it could be downward or upward. The latter case would indeed be possible if the ball had bounced on the floor between the initial and final time. \emph{Intuitively} we choose the negative sign (downward velocity) -- and we do that because in our mind we're actually applying the \emph{balance of momentum}, imagining the motion between the two times.
\item Strictly speaking the text does not tell us anything about the internal energy of the ball. Some process could take place -- think of the ball exploding -- that decreases the internal energy and leads to a higher kinetic energy and velocity. From the text this doesn't seem to be the case, but in a real-life physics inference we might be less certain.
\end{itemize}



\solution{sec:skaters_away}
...
Therefore let's first try to solve the problem considering the skaters as a \emph{single control volume}, with one balance of momentum and one balance of energy. In this approach, the fluxes of momentum and energy between the skaters are \emph{internal} to the control volume, and therefore they don't appear in the balances of momentum and energy.

\smallskip

The next question is which balances we need to consider in order to solve the problem. Remember, however, that \emph{all} balances apply and need to be satisfied for our control volume; in principle we could write all of them. Our question is not \enquote{which balances apply?} (answer: all of them!). Our question is \enquote{can we restrict our attention to one or few balances only, in order to find the information required in this specific problem?}.

A heuristic reasoning is this: the problem asks for \emph{two} numerical quantities: the horizontal velocity of each skater. We may therefore expect that \emph{two} balances might be enough. This is not guaranteed, maybe more are necessary; but it's a starting point. Intuitive options to consider are:
\begin{itemize}[nosep]
\item balances of $x$- and $z$-momentum;
\item balances of $x$--momentum and energy;
\item balances of $z$--momentum and energy.
\end{itemize}
...
\begin{itemize}
\item Newton's relation for momentum $\yP = \ym \yv$, to be written for initial and final times. This relation may need to be used together with the principle of \emph{extensivity}, because the skaters may not have the same velocity; that is, the velocity of matter is not the same throughout the control volume.
\item Energy for matter $\yE = \yU + \yEk + \yEp$, to be written for initial and final times.
\item Kinetic energy of matter $\yEk = \frac12 \ym \yv^{2}$, to be applied to each skater separately. Since the skaters may have different velocities, this relation may need to be used with the principle of extensivity of energy: we add up the kinetic energies of the two skaters.
\item Energy flux for matter $\yH = \yQ + \yF\cdot \yv$, which should be applied at all times between $\yti$ and $\ytf$.
\end{itemize}
...
\begin{itemize}
\item \enquote{together they have burned \qty{76}{J} of internal energy in total}: this refers to the \emph{difference} between the initial and final total energy: the final one is lower than the initial one by \qty{76}{J}. The total internal energy is, by extensivity, the sum of those of each skater.

\item No ice friction and negligible air means the total horizontal force $F_{x}$ on the two skaters -- viewed as one control volume -- is zero.

  Since the velocity of the skaters is purely horizontal, perpendicular to the normal force $\yFn$ on each skater, no friction also means that the total mechanical power $\yF\cdot \yv \equiv \yFn \cdot \yv$ on the each skaters is zero at all times.

\item No heat flux with the ice and negligible air means that the total heat flux $\yQ$ is also zero at all times.

\item Skaters have zero initial velocities.

\item Potential energy of each skater, and therefore of the whole, is constant.

\item Skaters' masses.
\end{itemize}

The full set of equations describing this physical situation can be written as follows, denoting one skater $a$ and the other $b$, and with $g=\qty{9.8}{m/s}$, $\yma = \qty{70}{kg}$, $\ymb = \qty{60}{kg}$:
\begin{equation*}
  \begin{aligned}
    & P_{x}(\ytf) = P_{x}(\yti)
    + \int_{\yti}^{\ytf}\!\!F_{x}(t)\dt
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$x$-momentum balance}}
    % \\
    % & P_{z}(\ytf) = P_{z}(\yti)
    % + \int_{\yti}^{\ytf}\!\!F_{z}(t)\dt
    % + \int_{\yti}^{\ytf}\!\!G_{z}(t)\dt
    % &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$z$-momentum balance}}
    \\
    & \yE(\ytf) = \yE(\yti)
    + \int_{\yti}^{\ytf}\!\!\yH(t)\dt
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy balance}}
    \\[1ex]
    & P_{x,a}(\yti) = \yma\, v_{x,a}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$x$-momentum $a$, initial time}}
    \\
    & P_{x,b}(\yti) = \ymb\, v_{x,b}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$x$-momentum $b$, initial time}}
    \\
    & P_{x}(\yti) = P_{x,a}(\yti) + P_{x,b}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}extensivity for momentum, initial time}}
    \\[1ex]
    & \yE(\yti) = \yU(\yti) + \yEk(\yti) + \yEp(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for matter, initial time}}
    \\
    & \yEka(\yti) = \tfrac12 \yma \yva(\yti)^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}kinetic energy $a$, initial time}}
    \\
    & \yEkb(\yti) = \tfrac12 \ymb \yvb(\yti)^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}kinetic energy $b$, initial time}}
    \\
    & \yEk(\yti) = \yEka(\yti) + \yEkb(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}extensivity for kinetic energy, initial time}}
    \\[1ex]
    & P_{x,a}(\ytf) = \yma\, v_{x,a}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$x$-momentum $a$, final time}}
    \\
    & P_{x,b}(\ytf) = \ymb\, v_{x,b}(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$x$-momentum $b$, final time}}
    \\
    & P_{x}(\ytf) = P_{x,a}(\ytf) + P_{x,b}(\ytf)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}extensivity for momentum, final time}}
    \\[1ex]
    & \yE(\ytf) = \yU(\ytf) + \yEk(\ytf) + \yEp(\ytf)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy content for matter, final time}}
    \\
    & \yEka(\ytf) = \tfrac12 \yma \yva(\ytf)^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}kinetic energy $a$, final time}}
    \\
    & \yEkb(\ytf) = \tfrac12 \ymb \yvb(\ytf)^{2}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}kinetic energy $b$, final time}}
    \\
    & \yEk(\ytf) = \yEka(\ytf) + \yEkb(\ytf)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}extensivity for kinetic energy, final time}}
    \\[1ex]
     & \yH(t) = \yQ(t) + F_{z}(t)\cdot v_{z}(t)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy flux for matter}}
     \\[1ex]
     & P_{z}(t) = \qty{0}{N\,s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on $z$-momentum}}
    \\
     & F_{x}(t) = \qty{0}{N}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on $x$-momentum flux}}
    \\
    & v_{a,z}(\yti) = \qty{0}{m/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
    & v_{b,z}(\yti) = \qty{0}{m/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info}}
    \\
    & \yU(\ytf) = \yU(\yti) - \qty{76}{J}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on internal energy}}
    \\
    & \yEp(\ytf) = \yEp(\yti)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on grav. potential energy}}
    \\
    & \yQ(t) = \qty{0}{J/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}info on heat flux}}
  \end{aligned}
...
This is just \emph{one example} of how we could write the physical laws and information about this physical system. When we write a set of equations like the one above, we must decide how detailed or how summarizing we want to be. In the system above we could have been even more detailed, for instance adding the balance of $z$-momentum and additional information on the surface \amp\ volume forces in the $z$-direction. Or we could have combined several equations beforehand instead.
...
\begin{equation*}
  \begin{gathered}
    \overbracket[0.5pt]{
      \yma\,v_{a,z}(\ytf) + \ymb\,v_{b,z}(\ytf)
    }^{P_{x}(\ytf)}
    {}={}
    \overbracket[0.5pt]{
      \yma\cdot(\qty{0}{m/s}) + \ymb\cdot(\qty{0}{m/s})
    }^{P_{x}(\yti)}
    {}+{}
    {\overbracket[0.5pt]{
      \qty{0}{N\,s}
    ...
    \begin{multlined}[][\linewidth]
      \overbracket[0.5pt]{
        \underbracket[0.5pt]{\yU(\yti) - \qty{76}{J}}_{\yU(\ytf)}
        + \tfrac12\yma\,v_{a,z}(\ytf)^{2}
        + \tfrac12\ymb\,v_{b,z}(\ytf)^{2}
        +{} \underbracket[0.5pt]{\yEp(\yti)}_{\yEp(\ytf)}
      }^{\yE(\ytf)}
      {}={}
      \\
      \overbracket[0.5pt]{
        \yU(\yti)
        + \tfrac12\yma\cdot(\qty{0}{m/s})^{2}
        + \tfrac12\ymb\cdot(\qty{0}{m/s})^{2}
        + \yEp(\yti)
      }^{\yE(\yti)}
      {}+{}
      \overbracket[0.5pt]{
...
\begin{equation*}
  \begin{aligned}
    ...
    \implies\qquad
    &\left\{\begin{aligned}
      &v_{a,z}(\ytf) = - \frac{\ymb}{\yma}v_{b,z}(\ytf)
      \\[1ex]
      &\yma \biggl[
      - \frac{\ymb}{\yma} \, v_{b,z}(\ytf)
      \biggr]^{2}
      + \ymb\,v_{b,z}(\ytf)^{2}
      = \qty{76}{J}
    \end{aligned}\right.
    ...
    &\left\{\begin{aligned}
      ...
      \pm\sqrt{
        ...
        } \approx \pm \qty{0.826}{m/s}
    \end{aligned}\right.
 \end{aligned}
\end{equation*}

The text says that skater $a$ (mass \qty{70}{kg}) is on the left. From this we can decide which sign to take, and finally arrive at
\begin{equation*}
  v_{a,z}(\ytf) \approx -\qty{0.71}{m/s} \ ,
  \qquad
  v_{b,z}(\ytf) \approx \qty{0.83}{m/s} \ .
\end{equation*}


\solution{sec:imagine_momentum_energy}
...
\begin{enumerate}[noitemsep]
\item The whole bomb has initially zero momentum. After the explosion, each individual fragment has non-zero momentum, but the sum of the momenta of all pieces is still zero.
\item We can take the bomb to be enough far away from stars or planets.
\item As the bomb breaks into different fragments during the explosion, there are fluxes of momentum and energy among the fragments. Such fluxes, however, are \emph{internal}: they are not fluxes of momentum and energy coming from outside the bomb or its fragments.
\item During the explosion, internal energy -- for instance of chemical origin -- get converted into kinetic energy of the pieces that fly away.
\item It is obvious that the kinetic energy of the bomb, still whole, is initially zero. After the explosion every fragment has a non-zero velocity, and therefore a positive kinetic energy. The sum of these positive kinetic energies is non-zero.
\end{enumerate}
Note in particular how the momentum is zero both before and after, but kinetic energy is not. This occurs because momentum is proportional to the velocity; velocities can have different directions and the vector sum can become zero. Kinetic energy is proportional to the \emph{square} of the velocity instead, so it is always positive and is a scalar. The sum of these positive scalars is non-zero.
...
\begin{equation*}
  ...
  + \int_{\yti}^{\ytf}\!\!F_{x}(t)\dt \ .
\end{equation*}
We directly omit the momentum supply $\yG$ because its $x$-component is zero.

\smallskip

The balance of energy requires the total energy  content in the control volume at the initial and final times. Cart and ramp have different velocities and positions, therefore we must use the principle of extensivity to find the total. The total will comprise internal, kinetic, and gravitational potential energies, as is usual for situations involving matter and no electromagnetic fields. The text says that the internal energy of cart \amp\ ramp is constant, so let's call it just $\yU$.

For the potential energy of the cart we can use the constitutive relation $\ym\,g\,z$, since the cart is viewed as a point; also at the final time this potential energy is zero because the $z$-coordinate of the cart is zero. We cannot use this constitutive relation for the ramp though, because the ramp has a large vertical extension. However, the ramp is only moving horizontally, not vertically, so its gravitational potential energy must be constant; let's call it $\yE_{\text{p,r}}$.
...
What about the energy flux $\yH(t)$ and the $x$-momentum flux $F_{x}(t)$? First let's see on which boundary surfaces of the control volume there are such fluxes.
...
The momentum flux, or surface force, from ice to ramp must have a $z$-component: the gravitational force pushes cart \amp\ ramp downward, so there must be a surface force from the ice to the cart that prevents the cart from sinking into the ice. The absence of friction between ice and ramp means that the surface force between them has no horizontal, $x$-component.

The energy flux, when matter is involved, comprises a heat flux $\yQ(t)$ and mechanical power $\yF(t)\cdot\yv(t)$. The text says that the heat flux is zero. For the mechanical power between ice and ramp we must consider that the force from the ice to the ramp is purely vertical, and the velocity of the ramp purely horizontal; therefore their dot-product is always zero: $\yF(t)\cdot\yv(t) = \qty{0}{J/s}$.
...
\begin{equation*}
  \begin{aligned}
    ...
    \\[1ex]
    & E(\yti) = \yU
    +{} \underbracket[0.5pt]{\yEk(\yti)}_{\qty{0}{J}} {}
    + \ym_{\text{c}}\,g\,z_{\text{c}}(\yti) + \yE_{\text{p,r}}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}initial total energy from extensivity}}
    \\
    & E(\ytf) = \yU
    + \tfrac12 \ym_{\text{c}}\,v_{\text{c},x}(\ytf)^{2}
    + \tfrac12 \ym_{\text{r}}\,v_{\text{r},x}(\ytf)^{2}
    + {}\underbracket[0.5pt]{\ym_{\text{c}}\,g\,z_{\text{c}}(\ytf)}_{\qty{0}{J}} + \yE_{\text{p,r}}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}final total energy from extensivity}}
    \\
    & P_{x}(\yti) = \qty{0}{N\,s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}initial $x$-momentum from extensivity}}
    \\
    & P_{x}(\ytf) =
    \ym_{\text{c}}\,v_{\text{c},x}(\ytf)
    +\ym_{\text{r}}\,v_{\text{r},x}(\ytf)
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}final $x$-momentum from extensivity}}
    \\
    &F_{x}(t) = \qty{0}{N}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}$x$-momentum flux}}
    \\
    &\yH(t) = \qty{0}{J/s}
    &&\qquad\mathrlap{\text{\footnotesize\color{midgrey}energy flux}}
  \end{aligned}
\end{equation*}
...
\begin{equation*}
  \begin{aligned}
    &\ym_{\text{c}}\,g\,z_{\text{c}}(\yti)
    =
    \tfrac12 \ym_{\text{c}}\,v_{\text{c},x}(\ytf)^{2}
    + \tfrac12 \ym_{\text{r}}\,v_{\text{r},x}(\ytf)^{2}
    \\[1ex]
    &\qty{0}{N\,s} = \ym_{\text{c}}\,v_{\text{c},x}(\ytf)
    +\ym_{\text{r}}\,v_{\text{r},x}(\ytf)
  \end{aligned}
\end{equation*}
where the unknowns are the final $x$-velocities $v_{\text{c},x}(\ytf)$, $v_{\text{r},x}(\ytf)$. This system can be solved with substitution, for instance writing $v_{\text{r},x}(\ytf)$ in terms of $v_{\text{c},x}(\ytf)$ in the second equation, then replacing it in the first:
\begin{equation*}
  \begin{aligned}
    &\left\{\begin{aligned}
      &\tfrac12 \ym_{\text{c}}\,v_{\text{c},x}(\ytf)^{2}
      + \tfrac12 \ym_{\text{r}}\,
      \biggl[
      -\frac{\ym_{\text{c}}}{\ym_{\text{r}}}\,v_{\text{c},x}(\ytf)
      \biggr]^{2}
      = \ym_{\text{c}}\,g\,z_{\text{c}}(\yti)
      \\[1ex]
      &v_{\text{r},x}(\ytf) =
    ...
    \pm \sqrt{\frac{
        ...
        \tfrac12 \ym_{\text{c}} + \tfrac12 \ym_{\text{c}}^{2} / \ym_{\text{r}}
      }}
    = \pm \qty{4.320}{m/s}
      \\[1ex]
      &v_{\text{r},x}(\ytf) =
...
\begin{equation*}
  v_{\text{c},x}(\ytf) \approx -\qty{4.3}{m/s} \ ,
  \qquad
  v_{\text{r},x}(\ytf) \approx +\qty{0.22}{m/s} \ .
\end{equation*}
...
\needspace{7\baselineskip}
\section{}\elabel{sec:forces_block_massless}

\marginpar{\vspace{-1\baselineskip}\centering\footnotesize\includegraphics[width=0.75\linewidth]{images/forces_block_2.jpg}%
%\\\flushleftright\color{mpcolor}%
}
%
An engineer has measured the surface forces applied on a block of a very light material at the points
\begin{equation*}
  \yra = [0.1, 0, 0.2]\,\unit{m}
  \quad\text{and}\quad
  \yrb = [-0.1, 0, -0.2]\,\unit{m} \ ,
\end{equation*}
...
Is the engineer's decision reasonable? \emph{(Hint: review the consequences of assuming zero mass for a piece of material, discussed in \chaps~10 and~12 of our lecture notes.)}



\needspace{10\baselineskip}
\section{}\elabel{sec:colliding_disks}

\marginpar{\vspace{-2\baselineskip}\centering\footnotesize\includegraphics[width=1\linewidth]{images/colliding_disks.jpg}%
\\\flushleftright\color{mpcolor}%
...
%
At some instant in time, two rigid disks denoted $a$ and $b$ of mass \qty{0.10}{kg} each, and constant moment of inertia $\yIc = \qty{0.0020}{kg\,m^{2}}$ each, are in motion in a gravitational field with gravitational acceleration \qty{9.8}{N/kg}. The mass-centres of both disks have $z$-coordinate \qty{0.0}{m}. Disk~$b$ has mass-centre velocity $[-1.0, 1.0]\,\unit{m/s}$. Disk~$a$ is not spinning (that is, its angular velocity is \qty{0}{rad/s}). Coordinates are $(t, x, z)$; angular velocities represent rotations around the $y$-direction and can be treated as scalars; also the moment of inertia can be treated as a scalar.

Half a second later, both disks have mass-centres at $z$-coordinate \qty{1.0}{m} and zero velocities, and neither disk is spinning (that is, both angular velocities are \qty{0}{rad/s}). The internal energy of disk~$a$ has increased by \qty{2.0}{J}, and that of disk~$b$ by \qty{1.0}{J}.
...
\begin{enumerate}[exerc]
\item Suppose we want to find the initial mass-centre velocity (both components) of disk~$a$, and the initial angular velocity of disk~$b$. Write down your reasoning about the control volumes, balance laws, constitutive relations, and physical principles that you need to use to solve this problem. Motivate shortly why they are needed. (\textbf{The important part is the reasoning and explanation; the final values are irrelevant.})

\item Find the numerical values of the initial mass-centre velocity of disk~$a$ and the initial angular velocity of disk~$b$.
\end{enumerate}
...
\solution{sec:forces_block_massless}

As we discussed in \chaps~10 and~12 of the lecture notes, assuming that an object has zero mass has important consequences for its momentum, angular momentum, and for the forces and torques applied to it. If two surface forces $\yFa$ and $\yFb$ are applied to it at points $\yra$ and $\yrb$, then there are two important consequences:
\begin{enumerate}[label=C\arabic*]
\item\label{item:C1flux_bis} The surface forces $\yFa$ and $\yFb$ must have equal magnitudes and directions, and opposite orientations, that is,
  \begin{equation*}
    \yFa = - \yFb \ .
  \end{equation*}

\item\label{item:C2flux_bis} The surface forces $\yFa$ and $\yFb$ must each have the same direction as the line connecting their surfaces of application:
  \begin{equation*}
    \yFa \ ,\enskip \yFb\ ,\enskip \yra-\yrb \quad \text{are parallel}
  \end{equation*}
\end{enumerate}

We see from the engineer's measurements, illustrated in the figure at the margin of the problem, that condition~\ref{item:C2flux_bis} is \emph{not} satisfied in this case. Therefore, assuming zero mass would break the balance of angular momentum.
...
\begin{equation*}
  \begin{aligned}
    \yM &= \yra \times \yFa + \yrb \times \yFb
    \\ &=
    \begin{bmatrix}
      0.1\\0\\0.2
    \end{bmatrix}\,\unit{m} \times
    \begin{bmatrix}
      0\\0\\3
    \end{bmatrix}\,\unit{N}
    +
...
But if the object has no mass, its momentum $\yP$ and angular momentum $\yL$ are always zero; and their time derivatives are also always zero. The volume forces $\yG$ and torques $\yTo$ are zero as well. So the balance of angular momentum would become
\begin{equation*}
  \begin{bmatrix}
    0\\0\\0
  \end{bmatrix}\,\unit{N\,m}
...
\paragraph{Text information.}

Given:
\begin{itemize}[nosep]
\item Two times $\yti$ and $\ytf$, known time-difference $\ytf-\yti = \qty{0.50}{s}$.
\item Disks are rigid.
\item Mass each disk: $\ym = \qty{0.10}{kg}$.
\item Moment of inertia each disk: $\yIc = \qty{0.0020}{kg\,m^{2}}$.
\item Initial mass-centre velocity disk~$b$: $\yvb(\yti) = [-1,1]\,\unit{m/s}$.
\item Initial angular velocity disk~$a$: $\omega_{a}(\yti) = \qty{0.0}{rad/s}$.
\item Initial mass-centre $z$-coord. disk~$a$: $z_{a} = \qty{0.0}{m}$.
\item Initial mass-centre $z$-coord. disk~$b$: $z_{b} = \qty{0.0}{m}$.
\item Final mass-centre velocity disk~$a$: $\yva(\ytf) = [0,0]\,\unit{m/s}$.
\item Final mass-centre velocity disk~$b$: $\yvb(\ytf) = [0,0]\,\unit{m/s}$.
\item Final angular velocity disk~$a$: $\omega_{a}(\ytf) = \qty{0}{rad/s}$.
\item Final angular velocity disk~$b$: $\omega_{a}(\ytf) = \qty{0}{rad/s}$.
\item Final mass-centre $z$-coord. disk~$a$: $z_{a} = \qty{1.0}{m}$.
\item Final mass-centre $z$-coord. disk~$b$: $z_{b} = \qty{1.0}{m}$.
\item Difference final-initial internal energy disk~$a$: $\yU_{a}(\ytf) - \yU_{a}(\yti) = \qty{2.0}{J}$.
\item Difference final-initial internal energy disk~$b$: $\yU_{b}(\ytf) - \yU_{b}(\yti) = \qty{1.0}{J}$.
\item No momentum or heat fluxes from environment to disks.
\end{itemize}

Required: $v_{a,x}(\yti)$, $v_{a,z}(\yti)$, $\omega_{b}(\yti)$.


\paragraph{Choice of control volumes.}

Two possible choices seem obvious at first sight: either consider each disk as a separate control volume, or consider both disks together as one control volume.

With the first choice -- two control volumes -- when we write the balance laws we would need to explicitly take into account any fluxes of momentum (surface forces), of energy, and of angular momentum (surface torques) between the two control volumes. The text of the problem, however, gives no information whatsoever about them. It might be possible to deal with them using the principle of symmetry of fluxes; but this would effectively correspond to viewing the two disks as one control volume only. Compare the solution on~\pageref{sec:tennisballs_collision_sol} to exercise~\ref{sec:tennisballs_collision}, where we had similar choices of control volumes and we tried both of them.

With the second choice -- a single control volume -- when we write the balance laws we don't need to explicitly take into account the fluxes of momentum, energy, angular momentum between the disks, because such fluxes are \emph{internal} to this control volume. Given that the text does not ask about these fluxes, it's convenient to treat them as internal. Let's therefore proceed by considering both disks together as one control volume.

\medskip

\emph{Example of short motivation:} We consider both disks together as one control volume, so that the interactions between the disks become internal and can be avoided.

\paragraph{Choice of balance laws.} The text asks to find three real-valued quantities: the two components of the initial mass-centre velocity of disk~$a$, and the initial angular velocity of disk~$b$. Heuristically we therefore expect \emph{three} balance laws to be necessary for finding the information required by the problem (although, of course, \emph{all} balance laws apply to the physics of this problem). The obvious list to choose from, in this 2D problem, is:
\begin{itemize}[nosep]
\item balance of $x$-momentum,
\item balance of $z$-momentum,
\item balance of energy,
\item balance of $y$-angular-momentum, which represents rotations in the $zx$ plane.
\end{itemize}
...
Velocities are also involved in the momentum (Newton's relation) and angular momentum (velocity of mass-centre) of matter, so the remaining two balances to use might be some combination of their balances. Which? The angular momentum of matter typically requires the \emph{position} of the mass-centre -- think of the constitutive relation $\yL = \yrcm \times \ym\yvcm + \yIc\,\yo$ for a rigid body -- and the \emph{position} of the surfaces where forces occur -- think of the constitutive relation $\yM = \yr \times \yF$. But the problem does not give full information about positions; only the $z$-coordinates. Therefore using the balance of angular momentum might lead to difficulties. So let's try using the balances of $x$- and $z$-momentum instead.

These balances are conveniently expressed in their integral form, since two time instants are involved.

\medskip

\emph{Example of short motivation:} The problem has three unknowns, so three balances seem necessary. It also involves internal energies, thus the balance of energy seems relevant. It also involves velocities but does \emph{not} give explicit \emph{positions}, thus two balances of momentum seem also appropriate. Integral expression is convenient because we have two time instants.

\paragraph{Constitutive relations}

The problem involves matter and rigid bodies in everyday conditions, and no electromagnetic fields. Therefore the constitutive relations that seem relevant are:
\begin{itemize}[nosep]
\item Newton's relation for momentum content: $\yP = \ym\yvcm$. It has to be used for each disk at the initial and final time instants.
\item Gravitational momentum supply near Earth's surface: $\yG = -\ym g [0,1]$. It has to be used at the initial and final time instants.
\item Constitutive relation for the total energy of a rigid body: $\yE = \yU + \tfrac12\ym\yvcm^{2} + \ym g \yzcm + \tfrac12 \yIc \omega^{2}$. It has to be used for each disk at the initial and final time instants.
\item Constitutive relation for total-energy flux: it says that the total-energy flux consists of heat flux and mechanical power, and both are zero according to the text. So the energy flux into the one control volume is zero.
\end{itemize}

\medskip

\emph{Example of short motivation:} This problem involves matter in everyday conditions and rigid bodies, so the usual constitutive relations 
for the momentum and total-energy content of a rigid body, momentum supply from gravity, and energy flux for matter.

\paragraph{Further principles}

Some of the constitutive relations above cannot be used directly on the whole control volume, because their conditions of validity are not satisfied. For example the relation $\yP = \ym\yvcm$ can only be used for \emph{one} rigid body, but the two disks taken together are not a rigid body. Similarly for the energy content. We must therefore use the \emph{extensivity} of momentum and energy to find the total. No use of flux symmetry seems necessary in this problem.

\medskip

\emph{Example of short motivation:} Using extensivity of momentum content and of energy content to find the total momenta and energies.

\subsubsection*{2.}

Let's collect together all equations from the previous physical analysis, remembering that constitutive equations usually have to be used for each time instant. Let's directly use the principle of extensivity for the totals:
\begin{equation*}
    \begin{aligned}
      & P_{x}(\ytf) = P_{x}(\yti) +
      {}\underbracket[0.5pt]{\int_{\yti}^{\ytf}\!\! F_{x} \dt}_{
        \qty{0}{N\,s}
      } {} +
      {}\underbracket[0.5pt]{\int_{\yti}^{\ytf}\!\! G_{x} \dt}_{
        \qty{0}{N\,s}
      }
    &&\qquad\text{\footnotesize $x$-momentum balance}
    \\
    & P_{z}(\ytf) = P_{z}(\yti) +
      {}\underbracket[0.5pt]{\int_{\yti}^{\ytf}\!\! F_{z} \dt}_{
        \qty{0}{N\,s}
      } {} 
    -{}\underbracket[0.5pt]{\int_{\yti}^{\ytf}\!\!(\ym g + \ym g) \dt}_{
      {}=2\ym g (\ytf-\yti) = \qty{0.98}{N\,s}
    }
    &&\qquad\text{\footnotesize $z$-momentum balance}
    \\
    & \yE(\ytf) = \yE(\yti) +
      {}\underbracket[0.5pt]{\int_{\yti}^{\ytf}\!\! \yH\dt}_{
        \qty{0}{J}
      }
    &&\qquad\text{\footnotesize total-energy balance}
    \\[1ex]
    &P_{x}(\yti) =
    \ym\, v_{a,x}(\yti) + {}\underbracket[0.5pt]{\ym\, v_{b,x}(\yti)}_{\qty{-0.10}{N\,s}}
    &&\qquad\text{\footnotesize $x$-momentum content, extensivity}
    \\
    &P_{x}(\ytf) =
    \ym\, v_{a,x}(\ytf) + \ym\, v_{b,x}(\ytf) = \qty{0}{N\,s}
    &&\qquad\text{\footnotesize $x$-momentum content, extensivity}
    \\[1ex]
    &P_{z}(\yti) =
    \ym\, v_{a,z}(\yti) + {}\underbracket[0.5pt]{\ym\, v_{b,z}(\yti)}_{\qty{+0.10}{N\,s}}
    &&\qquad\text{\footnotesize $z$-momentum content, extensivity}
    \\
    &P_{z}(\ytf) =
    \ym\, v_{a,z}(\ytf) + \ym\, v_{b,z}(\ytf) = \qty{0}{N\,s}
    &&\qquad\text{\footnotesize $z$-momentum content, extensivity}
    \\[1ex]
    &
    \begin{aligned}
      &E(\yti) = {}\\
      &\quad\yU_{a}(\yti) +
      \tfrac12\ym \bigl[v_{a,x}(\yti)^{2} + v_{a,z}(\yti)^{2}\bigr] +
      {}\underbracket[0.5pt]{\ym g z_{a}(\yti)}_{\qty{0}{J}}{} +
      \tfrac12 \underbracket[0.5pt]{\yIc \omega_{a}(\yti)^{2}}_{\qty{0}{J}} {}
      +{}\\
      &\quad\yU_{b}(\yti) +
      \tfrac12\ym \bigl[\underbracket[0.5pt]{v_{b,x}(\yti)^{2} + v_{b,z}(\yti)^{2}}_{\qty{2}{m^{2}/s^{2}}}\bigr] +
      {}\underbracket[0.5pt]{\ym g z_{b}(\yti)}_{\qty{0}{J}}{} +
      \tfrac12 \yIc \omega_{b}(\yti)^{2}
    \end{aligned}
    &&\qquad\text{\footnotesize energy content, extensivity}
    \\[1ex]
    &
    \begin{aligned}
      &E(\ytf) = {}\\
      &\quad\yU_{a}(\ytf) +
      \tfrac12\underbracket[0.5pt]{\ym \bigl[v_{a,x}(\ytf)^{2} + v_{a,z}(\ytf)^{2}\bigr]}_{\qty{0}{J}} +
      {}\underbracket[0.5pt]{\ym g z_{a}(\ytf)}_{
        %=\num{0.10}\cdot\num{9.8}\cdot\num{1.0}\ \unit{J}
       \qty{0.98}{J}
        } {} +
      \tfrac12 \underbracket[0.5pt]{\yIc \omega_{a}(\ytf)^{2}}_{\qty{0}{J}}{}
      +{}\\
      &\quad\yU_{b}(\ytf) +
      \tfrac12\underbracket[0.5pt]{\ym \bigl[v_{b,x}(\ytf)^{2} + v_{b,z}(\ytf)^{2}\bigr]}_{\qty{0}{J}} +
      {}\underbracket[0.5pt]{\ym g z_{b}(\ytf)}_{
        %=\num{0.10}\cdot\num{9.8}\cdot\num{1.0}\ \unit{J}
        \qty{0.98}{J}
        } {} +
      \tfrac12 \underbracket[0.5pt]{\yIc \omega_{b}(\ytf)^{2}}_{\qty{0}{J}}{}
    \end{aligned}
    &&\qquad\text{\footnotesize energy content, extensivity}
  \end{aligned}
\end{equation*}
where $\ym$, $g$, $\yIc$ are known.

Substituting the last equations into the first three, we get
\begin{equation*}
  \begin{aligned}
    &\qty{0}{N\,s} = \ym\, v_{a,x}(\yti) - \qty{0.10}{N\,s}
    \\
    &\qty{0}{N\,s} = \ym\, v_{a,z}(\yti) + \qty{0.10}{N\,s}
    - \qty{0.98}{N\,s}
    \\
    &
    \begin{aligned}
      &\yU_{a}(\ytf) +\qty{0.98}{J} +
      \yU_{b}(\ytf) +\qty{0.98}{J}
      ={}
      \\&\quad
      \yU_{a}(\yti) +
      \tfrac12\ym \bigl[v_{a,x}(\yti)^{2} + v_{a,z}(\yti)^{2}\bigr] +
      \yU_{b}(\yti) + \qty{0.10}{J} +
      \tfrac12 \yIc \omega_{b}(\yti)^{2}
    \end{aligned}
  \end{aligned}
\end{equation*}
where\enskip$\ym = \qty{0.10}{kg}$\,,\enskip$\yIc=\qty{0.0020}{kg\,m^{2}}$\,,\enskip and\enskip$\yU_{a}(\ytf) + \yU_{b}(\ytf) - \yU_{a}(\yti) - \yU_{b}(\yti) = \qty{3}{J}$\enskip have known values. The solution is
\begin{equation*}
  v_{a,x}(\yti) \approx \qty{1.0}{m/s} \ ,
  \quad
  v_{a,z}(\yti) \approx \qty{8.8}{m/s} \ ,
  \quad
  \omega_{b}(\yti) \approx \pm\qty{31}{rad/s} \ ;
\end{equation*}
