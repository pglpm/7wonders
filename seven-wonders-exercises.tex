...
%% Last-Updated: 2025-02-27T01:57:49+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anontrue
\newif\ifarxiv
...
\item\label{item:team_A} One group of engineers show that if you keep the original constitutive relation~\eqref{eq:const_block}, but modify the conservation law~\eqref{eq:energy_block} as follows:
  \begin{equation}
    ...
    \biggl[\frac{\di\yE(t)}{\dt}\biggr]^{2} = \yH(t)
  \end{equation}
...
\item  We consider an imaginary and static closed control surface enclosing the region where the laser beam (the electromagnetic field) was initially.
  ...
  \begin{equation*}
    \yL(\ytf) = \yL(\yti) + \int_{\yti}^{\ytf}\!\!\yto(t)\, \dt 
  \end{equation*}
of which two are known. We can find the \textbf{ef}flux by algebra:
\begin{equation*}
  \begin{split}
   - \int_{\yti}^{\ytf}\!\!\yto(t)\, \dt &=    \yL(\yti) - yL(\ytf)
...
Start from script for the imaginary oscillating reaction discussed in \sects~6.2 and~6.5 of the main text (Octave: \furl{https://pglpm.github.io/7wonders/code/oscillatingreaction.m}{\texttt{oscillatingreaction.m}}, Python: \furl{https://pglpm.github.io/7wonders/code/oscillatingreaction.py}{\texttt{oscillatingreaction.py}}), and modify it to simulate different constitutive relations, or change the chemical system a little. Describe what happens, and try to explain why that happens from the mathematical form of the constitutive relations. Keep in mind that in some situations the simulation may become numerically unstable and even crash. Also check what are the correct dimensions of the constants that appear in your physical laws.
...
Besides the required answer, we also found that positive current is flowing from the top to the bottom capacitor.
  ...
  \medskip
  \marginpar{\vspace{0.5\baselineskip}\centering\footnotesize\includegraphics[width=0.75\linewidth]{images/capacitors_CS2.jpg}%
  }%
...
For this exercise, download the script for the falling tennis ball (Octave: \furl{https://pglpm.github.io/7wonders/code/tennisball_rP.m}{\texttt{tennisball\_rP.m}}, Python: \furl{https://pglpm.github.io/7wonders/code/tennisball_rP.py}{\texttt{tennisball\_rP.py}}), and make first sure it runs correctly.
...
For this exercise, use again the script for the falling tennis ball (Octave: \furl{https://pglpm.github.io/7wonders/code/tennisball_rP.m}{\texttt{tennisball\_rP.m}}, Python: \furl{https://pglpm.github.io/7wonders/code/tennisball_rP.py}{\texttt{tennisball\_rP.py}}).
...
\item Modify the tennis-ball script so that it simulates two tennis balls instead of one.
  ...
  \item Try to modify the plots so that the vertical positions of the two balls are plotted against time.
  \end{itemize}
...
Call $\yra$ and $\yrb$ the positions of the two tennis balls, and $\yFa$ the \textbf{in}flux of momentum into tennis ball $a$ from $b$. Let's the following constitutive relation:
\begin{equation*}
  \yFa =
  \begin{dcases}
    [0, 0, 0]\,\unit{N}
...
Start from the script simulating two independent tennis balls, and implement the constitutive relation from the previous exercise, by means of an \texttt{if-else} construct. In particular:
...
\item See for instance these scripts: Octave \furl{https://pglpm.github.io/7wonders/code/tennisball_rP_2obj.m}{\texttt{tennisball\_rP\_2obj.m}}, Python \furl{https://pglpm.github.io/7wonders/code/tennisball_rP_2obj.py}{\texttt{tennisball\_rP\_2obj.py}}.
\end{enumerate}
...
\item The physical dimensions appearing in the equation can be written as follows:
  \begin{equation*}
    \texttt{mom.flux} = \frac{k}{\texttt{length}^{2}}\cdot\texttt{length}
  \end{equation*}
  therefore $k$ has dimension $\texttt{mom.flux} \cdot \texttt{length}$.
...
\item See for instance these scripts: Octave \furl{https://pglpm.github.io/7wonders/code/tennisball_rP_2obj_collision.m}{\texttt{tennisball\_rP\_2obj\_collision.m}}, Python \furl{https://pglpm.github.io/7wonders/code/tennisball_rP_2obj_collision.py}{\texttt{tennisball\_rP\_2obj\_collision.py}}.

  Here are the plots we obtain from them:
  \begin{center}
    \includegraphics[width=\linewidth]{images/tennisball_collision.pdf}%
  \end{center}

\item See for instance these scripts: Octave \furl{https://pglpm.github.io/7wonders/code/tennisball_rP_2obj_collision_floor.m}{\texttt{tennisball\_rP\_2obj\_collision\_floor.m}}, Python \furl{https://pglpm.github.io/7wonders/code/tennisball_rP_2obj_collision_floor.py}{\texttt{tennisball\_rP\_2obj\_collision\_floor.py}}.

  Here are the plots we obtain from them:
  \begin{center}
    \includegraphics[width=\linewidth]{images/tennisball_collision_floor.pdf}%
  \end{center}
\end{enumerate}
