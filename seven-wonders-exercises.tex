...
%% Last-Updated: 2026-03-12T21:48:34+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anonfalse
\newif\ifarxiv
...
%
Two people are trying to move a crate of \masse\ \qty{110}{kg}, as illustrated in the margin figure. The person on the left exerts a diagonal surface force $[350, 200]\,\unit{N}$; the one on the right, a horizontal surface force $[250, 0]\,\unit{N}$. The static-friction factor between the floor and the crate is $0.62$, and the gravitational acceleration is $g = \qty{9.8}{N/kg}$.

Using Amontons's law for static friction, determine if the two people manage to set the crate into motion.
...
First let's find the unit normal to the contact surface between ground and crate, since it isn't given by the problem and it's necessary for defining what we mean by \enquote*{normal} and \enquote*{tangential} components. Given the geometry of the problem, the unit normal is
\begin{equation*}
  \yns = [0, 1] \ .
\end{equation*}

Then let's proceed by steps:
...
\item The gravitational force, given by the usual constitutive relation:
  \begin{equation*}
    \yG = -\ym g [0, 1] = -[0, \num{1078}]\,\unit{N} \ .
  \end{equation*}
\item The contact force from ground to crate, which we write as the sum $\yFn + \yFs$ of a normal component and a tangential static-friction component.
\end{itemize}
...
\begin{equation*}
  ...
  \\
  \implies\quad&
  \yFn = -(\yFa \cdot \yns)\,\yns
  -(\yFb \cdot \yns)\,\yns - (\yG \cdot \yns)\,\yns
  \\
  \implies\quad&
  \yFn = -[0, 200]\,\unit{N} - [0,0]\,\unit{N} - [0, -1078]\,\unit{N}
  \\
  \implies\quad&
  \yFn = [0, 878]\,\unit{N} \ .
  \end{split}
...
\paragraph{Step~3.}
Now that we have the normal component of the contact force from ground to crate, we can calculate the maximum value that static friction can have:
\begin{equation*}
...
\begin{equation*}
  \qty{600}{N} = \abs{\yFr} >
  \yfris\, \abs{\yFn} = \qty{544}{N} \ .
\end{equation*}
Which means that the two people do manage to set the crate into motion.
