...
%% Last-Updated: 2026-02-19T11:06:20+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anonfalse
\newif\ifarxiv
...
Finding the final velocity of bullet \amp\ block is straightforward if we consider them as a single (deformable) object, or more precisely as a single control volume.

Denote by $P$ momentum in this control volume, by $F$ the total surface force (momentum influx) on it, and by $G$ the total volume force. All these quantities are meant as $x$-components.

The integral expression of the balance of momentum can be applied to this single control volume:
\begin{equation*}
  P(\ytf) = P(\yti)
  + \int_{\yti}^{\ytf}\!F(t)\dt
  + \int_{\yti}^{\ytf}\!G(t) \dt \ .
...
At the final time $\ytf$ all matter (bullet \amp\ block) in the control volume is moving with the same velocity $v_{\textrm{f}}$, so we can apply Newton's constitutive relation for momentum:
\begin{equation*}
  P(\ytf) = \ym\, v_{\textrm{f}}
  = (\ym_{\textrm{bul}} + \ym_{\textrm{blo}})\, v_{\textrm{f}}
  = (\qty{0.208}{kg})\cdot v_{\textrm{f}}
\end{equation*}
where $\ym$ is, by the principle of extensivity of \masse, the sum of the mass $\ym_{\textrm{bul}}$ of bullet and the mass $\ym_{\textrm{blo}}$ of the block.

Let's see if we can calculate the three terms on the right side of the momentum balance.

For the initial momentum $P(\yti)$ we cannot apply Newton's relation directly, because the matter in the control volume doesn't have a common velocity: the bullet is moving, but the block is at rest. But we can use the principle of extensivity, considering the bullet and the block as two sub-volumes, and find the total momentum by summing their momenta. These momenta can be found with Newton's relation because the matter velocity is the same in each:
\begin{equation*}
  \begin{gathered}
    ...
    \ ,
    \qquad
    P_{\textrm{blo}}(\yti) = \ym_{\textrm{blo}}\cdot \qty{0}{m/s} = \qty{0}{N\,s} \ ,
    \\[2ex]
    P(\yti) = P_{\textrm{bul}}(\yti) + P_{\textrm{blo}}(\yti) = \qty{2.96}{N\,s} \ .
  \end{gathered}
\end{equation*}

Between times $\yti$ and $\ytf$ the bullet and block are exerting surface forces on each other; but considered as a single, there are no external surface forces exerted on their single control volume. Also the gravitational force is zero. Therefore the time-integrated terms are
\begin{equation*}
  \int_{\yti}^{\ytf}\!F(t)\dt = \int_{\yti}^{\ytf}\!(\qty{0}{N})\dt = \qty{0}{N\,s}
  \ ,
\qquad
  \int_{\yti}^{\ytf}\!G(t)\dt = \int_{\yti}^{\ytf}\!(\qty{0}{N})\dt = \qty{0}{N\,s}
  \ .
\end{equation*}

Substituting all expressions above in the balance of momentum we finally find
\begin{equation*}
  \begin{aligned}
    &&
      P(\ytf) &= P(\yti)
