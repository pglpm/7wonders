...
%% Last-Updated: 2025-03-07T08:23:43+0100
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifanon
\anontrue
\newif\ifarxiv
...
\begin{equation*}
  a = \yE/\yT^{4} \,
\end{equation*}
which means that $a$ has physical dimension $\textsf{energy}/\textsf{temperature}^{4}$ or equivalently $\textsf{mass}\cdot\textsf{length}^{2}/(\textsf{time}^{2}\cdot\textsf{temperature}^{4})$. Possible units are therefore \enquote*{\unit{J/K^4}}.

An analogous procedure shows:
\begin{itemize}
\item $b$: physical dimension $\textsf{energy}^{-1}$, units \enquote*{\unit{J^{-1}}}.
\item $d$: physical dimension $\textsf{temperature}$, units \enquote*{\unit{K}}.
\item $R$: physical dimension $\textsf{momentum}\cdot\textsf{length}/(\textsf{substance}\cdot\textsf{temperature})$, units \enquote*{\unit{N\,m/(mol\,K)}}.
\item $\yU_{0}$: physical dimension $\textsf{energy}$, units \enquote*{\unit{J}}.
\item $C$: physical dimension $\textsf{energy}/(\textsf{substance}\cdot\textsf{temperature})$, units \enquote*{\unit{J/(mol\,K)}}.
\end{itemize}
