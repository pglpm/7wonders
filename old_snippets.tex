\begin{exercise}[label={ex:twobooks}]
  Two identical books, on top of each other, rest on a table. Each book has  \masse\ \qty{0.2}{kg}. Choose two static closed control surfaces of cuboid shape, each wrapping one book, and having one side in common (where the two books touch). Assume that the momentum supply in each control volume is $[0\,,0\,,\num{-2}]\,\unit{N}$, constant in time, and make the same assumptions as before regarding the momentum flux across the parts of the surfaces in contact with air.
...
The total amount of momentum in this control volume is zero and constant:
\begin{equation*}
  \yP(t) = [0\,,0\,,0]\,\unit{N\,s} \ ,
  \qquad \frac{\di\yP}{\dt} = [0\,,0\,,0]\,\unit{N} \ .
\end{equation*}
...
\end{equation*}
Then \emph{what are the momentum fluxes across different parts of the control surface?}
...
\begin{equation*}
  \begin{split}
    \yF(t) &= \underbracket[0pt]{\frac{\di\yP(t)}{\dt}}_{\color{grey}[0,0,0]\unit{N}\text{ because constant}} - \underbracket[0pt]{\yG(t)}_{\color{grey}[0,0,\num{-2}]\,\unit{N}}
\\[3\jot]
 &=   [0\,,0\,,\num{2}]\,\unit{N}\enskip\text{(constant)}.
  \end{split}
\end{equation*}
Note that this is the only piece of information that the balance of momentum, applied to the chosen closed control surface, can give us. The influxes \autoref{sec:in_out_flux}{through different parts} of the control surface could be very different from this value, which is only their total. But the balance of momentum by itself cannot give us these partial fluxes. We need additional information, which can only come from direct measurements or from constitutive relations.
...
\begin{equation*} \yF_{\text{air}\rightsquigarrow\text{book}}(t) \approx [0\,,0\,,0]\,\unit{N} \ .
...
\begin{equation*}
  \yF(t) = \yF_{\text{air}\rightsquigarrow\text{book}}(t) + \yF_{\text{table}\rightsquigarrow\text{book}}(t) \ .
\end{equation*}
From the last three equations we finally find
\begin{equation*} \yF_{\text{table}\rightsquigarrow\text{book}}(t) \approx [0\,,0\,,\num{2}]\,\unit{N}\enskip\text{(constant)}.
\end{equation*}
