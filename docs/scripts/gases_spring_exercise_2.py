...
from numpy.linalg import norm ; from numpy import array as vector
...
Ln = 0.1                 # m: natural length spring
k = 300                  # N/m: elastic constant spring
rs = vector([0, 0.2])    # m: position fixed-end spring
rg1 = vector([-0.2, 0])  # m: position fixed-end gas 1
rg2 = vector([0.2, 0])   # m: position fixed-end gas 2
...
t = 0                   # s: time
r = vector([0.2, 0.1])  # m: position object
v = vector([0, 0])      # m/s: velocity object
T1 = 298.15             # K: temperature gas 1
T2 = 298.15             # K: temperature gas 2

## Boundary conditions
G = -m * g * vector([0, 1])  # N: momentum supply object
...
clf
subplot(3, 1, 1)
plot(r[0], r[1], 'o', color='#000000'); 
plot([rs[0], r[0]], [rs[1], r[1]], '--', color='#CCBB44')
plot([rg1[0], r[0]], [rg1[1], r[1]], '-', color='#66CCEE')
plot([rg2[0], r[0]], [rg2[1], r[1]], '-', color='#66CCEE')
xlim([-0.7, 0.7]); ylim([-0.7, 0.7]); gca().set_aspect("equal", adjustable="box")
