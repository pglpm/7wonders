...
ya0 = -3; za0 = 0; % initial position of object a
yb0 = 3; zb0 = 0; % initial position of object a
vya0 = 2; vza0 = 2; % initial velocity of object a
vyb0 = 1; vzb0 = 1; % initial velocity of object b
...
while sign(dt)*t < sign(dt)*t1 % this check allows for backward time integration
  %% update time
  t = t + dt;
  %% Calculate force by spring with Hooke's law
  Fyab = -k*(ya-yb);
  Fzab = -k*(za-zb);
  %% update momentum
  Fyba = -Fyab;
...
%% Plot ya vs t
figure();
plot(tSave,yaSave,'-','Color',cols(1,:));
axis('tight'); xlabel('{\it t}/s'); ylabel('{\it y}/m');
