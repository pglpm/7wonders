...
mu = 0.8; % kinetic friction coefficient
g = 9.8; % gravitational acceleration

t_end = 11; % final time
dt = 0.0001; % time step

G = -m*g; % gravitational supply of momentum
Fn = 5; % normal force on piston
...
while t < t_end
  P = m*v;
  F_pis_fri = -sign(v) * mu * Fn;
  F_pis_gas = A * ( N*R*T/(A*z) - mu*A*v/(A*z) );
  F_pis_tot = F_pis_gas + F_pis_fri;
  E = C*N*T;
  Q = A*h*(T_out - T);
  F_gas_pis = -F_pis_gas;
  phi = Q + F_gas_pis * v;
