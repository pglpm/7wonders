...
%% Last-Updated: 2026-06-29T18:43:08+0200
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifarxiv
\arxivfalse
...
\newcommand*{\firstdraft}{18 November 2025}
\newcommand*{\firstpublished}{\firstdraft}
\newcommand*{\updated}{\ifarxiv***\else\today\ [draft]\fi}
\newcommand*{\propertitle}{The definition of energy (and momentum)\\ depends on a fiducial vector field%
}% title uses LARGE; set Large for smaller
\newcommand*{\pdftitle}{\propertitle}
\newcommand*{\headtitle}{Fiducial 4-vector fields}
...
\else\settypeblocksize{*}{26.1408pc}{1.618}% nearer to a 66-line newpx and preserves GR
\fi
...
% gamba1967
% ehlers1971
% gotayetal1992 % \sect~3.2 p.~62
% choquetbruhatetal1989_r2000 % \sect~II.7.III p.~87
% choquetbruhatetal1989_r2000 % \sect~III.7.III \eqns~(5), (6)
% hawkingetal1973_r1994 % \sect~3.2 p.~62
% hawkingetal1973_r1994 % \sect~3.2 p.~62
% misneretal1970_r2017 % \chap~2 Box~5.4
% weinberg1972 % \sect~4.11
% malament2012 % \sect~2.5
% vandantzig1934b % part~4 \sect~1
% landauetal1939_t1996 % \chap~2 \sect~14
% taub1967 % express equivalence of div.eqn. with contraction with vector
%
% % Check also
% ~/Eureka/pglpm241222-notes_energy_momentum.tex
% szabados2004_r2009 % \chap~2
% penroseetal1965
% ehlersetal1965
% waldetal2000
...
\section{Questions about energy}
\label{sec:intro}

The present note discusses two sets of questions that at first sight might not appear to be connected, also because one set is typical of Newtonian mechanics and appears already at an undergraduate level, while the other appears very naturally in general relativity. But it will be shown that the questions of the two sets are instead deeply connected and their answers can be traced to a common idea.

\begin{enumerate}[label=A\arabic*.,ref=A\arabic*]
\item\label{item:def_total_energy} What is the total energy of an extended body? Should it include internal, kinetic, gravitational potential energy? or only some of these?
\item\label{item:frame_indifference_energy} Why is internal energy the same in all frames, whereas kinetic energy is frame-dependent?
\item\label{item:virtual_work} Why is it possible to derive physical laws about energy by consider \enquote{virtual displacements}?
\end{enumerate}

\begin{enumerate}[label=A\arabic*.,ref=A\arabic*,start=4]
\item\label{item:bernoulli_theorem}Is Bernoulli's theorem a consequence of the balance of energy? or of the balance of momentum?
\end{enumerate}

\begin{enumerate}[label=B\arabic*.,ref=B\arabic*]
\item\label{item:integrate_energy} How can we associate a total energy to a region of spacetime?
\end{enumerate}



%% diverse transf. properties
%% different flow situations
%% why F appears in energy flux
%% double-dependence on coords
%% gravitation as energy or work
%% is it tensor or 4-vector?
%% virtual work
%% connection with spacelike integration surfaces:
%% gamba1967 groen1975c
%% rohrlich1960
...
\begin{enumerate}
\item Why is internal energy invariant under a change of Newtonian reference frame, even non-inertial, whereas kinetic energy is not?

\item Consider a car accelerating on a street, say in the positive-$x$ direction. In the frame of reference where the street is at rest, there is a flow of positive $x$-momentum -- that is, force -- from the street to the car, or equivalently a flow of negative $x$-momentum from the car to the street. In the frame of reference where the car is at rest, the \emph{flow} of momentum is the same.

  Not so with energy flow. In the frame of reference where the street is at rest, there is no flow of energy between street and car: the contact forces involved act on matter that has momentarily zero velocity, so their work is zero. In the reference frame where the car is at rest, there is a flow of positive energy from the car to the street: the contact forces do work on the latter.

  Why do the flows of momentum and of energy behave differently under this change of frame?

\item In fluid mechanics, and more generally in continuum mechanics, the balance of energy can be expressed in very different forms. In one, only the time derivative of the internal energy appears, and kinetic energy is mysteriously absent; this expression of energy balance is moreover invariant with respect changes of frames, even non-inertial. In another, kinetic energy does appear; this expression is not invariant with respect to changes of frame.

  Why do we have this kind of flexibility and possible invariance with the balance of energy, but not with the balance of momentum?
\end{enumerate}
